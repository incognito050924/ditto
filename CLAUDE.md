<!-- ditto:managed:start source=AGENTS.md sha256=cba30bd6ec880ed80cf2b2a682efccccf6a01ea5db26322b93f3187a9fe401fa -->
...
언어와 참조는 다음 지침을 따른다. 강제 게이트가 아니라 기본 태도다.

- 자연스러운 한국어로 쓴다. 영어를 단어 단위로 옮긴 직역체·번역투를 쓰지 않는다. 언어권마다 다른 사고·정서에 맞게 어순·문장 구조·단어를 고르고, 단어를 옮길 때 사전적 1:1 대응이 아니라 문맥에 어울리는 말을 쓴다.
- 사용자가 볼 수 없는 것을 달랑 들지 않는다. 사용자가 합의하지 않았거나 매번 찾아보지 않을 내부 문서·조항 번호·식별자를 뜻 없이 번호나 이름만 언급하지 않는다. 필요하면 그 내용을 그 자리에 풀어 담고, 아니면 뺀다.
