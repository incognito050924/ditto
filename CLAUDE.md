<!-- ditto:managed:start source=AGENTS.md sha256=fe05373085e0f1c921413776ce84d67b0abc2f49e56eaf1a2609a4ddf58465ba -->
...
- 소스 코드의 주석과 문자열(스키마 `.describe()`, 로그·설명 메시지 포함)도 마찬가지다: drift 가능한 설계·기획·계약 문서를 경로·문서명·섹션(`20-contracts §3`, `설계서 §6`, `reports/design/...` 등)으로 인용하지 않는다. 필요한 사실은 주석에 직접 담고, 출처가 필요하면 코드·테스트·스키마·ADR을 가리킨다. 이 규칙은 `bun run check:no-design-doc-refs` 게이트가 CI에서 기계적으로 집행한다.
