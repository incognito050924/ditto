...
3. A hard mechanical filter (`filterHealPatch`, `src/core/e2e/healer-policy.ts`) is the real guarantee: it re-checks every hunk and drops any that touches a forbidden token even if you proposed it. Only the filtered `allowed` hunks may ever be applied. After a selector heal is applied, the touched steps are force-reflagged for re-review — a heal can never silently re-green a reviewed assertion.

This MUST-NOT list is the soft layer. Treat it as absolute anyway; the filter exists because obedience alone is not a guarantee.
