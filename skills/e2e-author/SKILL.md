...
When a generated test flakes on a stale selector, healing is bounded to selector and wait changes; an expected value or an auto-skip is never touched. This bound is carried by the constrained healer def — a prompt-level constraint the healer agent follows — not by a mechanical post-filter.
- Default is **propose-only**: the healer writes a patch to `.ditto/local/runs/<id>/heal-proposals/*.patch` with a rationale and STOPS — it never applies or commits; the DSL author reviews the proposed selector/wait change before it lands, and re-reviews any assertion the heal touched.
