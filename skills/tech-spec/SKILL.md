...
2. **`ditto:question-generator` × N (parallel, fresh each round)** — fan out N generators (N = the resolved `--generators`, default 2, range 1..6) in **one parallel batch — a SINGLE message carrying all N Task calls, never one-at-a-time** (sequential spawns serialize the round latency N-fold; the fan-out speeds the round up only if the calls run concurrently), each with **only** the minimal packet `{fixed facts & decisions, project status/environment, current draft / target empty section}`. Excluded: the interview narrative and your own guesses — that exclusion is the anti-bias mechanism. Independent generators cover each other's blind spots.
3. **`ditto:question-gate` (fan-in)** — one gate over the pooled candidates; it scores (consensus / quality / necessity / answer_value) and selects, or signals dry. **When N=1** the consensus axis is moot, so you MAY score the single candidate against the threshold yourself instead of a separate gate spawn — but keep the dry/threshold judgment honest (only the redundant consensus spawn is dropped, not the scoring).
...
4. If `selected` is non-empty, handle it per the resolved `gate_mode` (the gate itself never asks the user — this is the driver's step). **Presentation contract — whenever a question reaches the user (here or via `draft` escalation):**
   1. **Structural gate (ac-6).** First gate the selected candidate with `ditto tech-spec check-question --json '{…scored question…}'` — the tech-spec surface of the shared presentation-contract validator (non-zero exit when the candidate lacks a plain-language `user_explanation`/`why_matters` or leaks an un-glossed internal identifier on the user-reaching face). Send a rejected candidate back to the generators; do **not** ask it.
   2. **Session-blind review of value/irreversible/domain questions (ac-4).** A question whose answer is an irreversible, product-value, or domain-meaning call (tech-spec's `critical`) reaches the user only after the session-blind `ditto:context-reviewer` clears it — the curse-of-knowledge catch the grounded generator/gate structurally cannot make. Spawn it (Task, one level deep) with ONLY the user-reaching surface (`text` + `user_explanation` + option labels) — never the spec doc / fixed facts / grounding (session-blind *is* the check); it returns `{verdict, reason}`, read-only. On `reject`, regenerate with its `reason` noted, up to the per-question regeneration cap `REVIEW_REGENERATE_CAP = 2` (fixed and small). **Terminal fallback — honesty over silence (ADR-0018 D2):** when the host cannot spawn the reviewer (capability absent) OR the cap is exhausted with no pass, ask the question **flagged as `unverified-degraded` context** — never a silent ask as if reviewed, a silent drop, or a stall. You (the driver) are the sole writer of that outcome into the fixed-facts ledger; parallel reviews fold back **sequentially** so no two writes clobber it.
   3. **Briefing first when the context overflows (ac-5).** Test the rendered `user_explanation` (else `text`) against `needsBriefing` (the shared `OPTION_DESCRIPTION_BUDGET` threshold in `src/core/question-context.ts`); when it overflows the compact option UI, present that context as a short **briefing in the conversation body FIRST**, then ask — never cram a long explanation into an option `description`.
   4. **Ask in the user's language** with that `user_explanation` (plain *why we ask + what your answer decides*) — never raw code, `file:line`, schema fields, or axis names.
...
- A value/irreversible/domain question reaches the user only after the session-blind `ditto:context-reviewer` passes it (within the cap of 2) or is asked flagged `unverified-degraded` — never silently asked as if reviewed, dropped, or stalled (ADR-0018 D2). The reviewer is read-only; only the driver writes the outcome.
