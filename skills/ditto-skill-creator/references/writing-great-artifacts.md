...
The authoring craft shared by **both** ditto creators. A skill and an autopilot
owner subagent are the same kind of thing — a **prompt-context artifact**: text
loaded into an agent's context to steer it. So the principles below are authored
once, here, and pointed at from both `ditto-skill-creator` and
`ditto-agent-creator` (single source of truth — the artifacts we ship should obey
the rule they teach). Surface-specific rules stay in each creator's own
`*-conventions.md`; this file is the common core.
...
When a ditto authoring convention (in `ditto-skill-conventions.md` /
`ditto-agent-conventions.md`, or an old habit baked into an existing skill)
conflicts with a principle here — or dilutes its effect — **this craft wins**. The
