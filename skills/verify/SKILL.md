...
## Re-verifying a stuck criterion (`ditto verify --criterion`)

`ditto verify <wi> --criterion <ac> -- <command>` supersedes a criterion's verdict with fresh evidence from a command run now. Legitimate uses only:

- **close-ordering deadlock** — the criterion text encodes a process effect the done path itself performs (e.g. "issue #N is closed"; deep-interview forbids authoring such ACs). The done gate requires all-pass before it closes the issue, so that criterion can never pass on its own. Re-verify the substantive fact the criterion stands for with fresh evidence; the process effect then happens as the tail of `done`.
- **retroactively unmeetable form** — the criterion demands a form of evidence that can no longer be produced (e.g. an artifact only a past run could have emitted), while the substantive behavior is verifiable now. Verify that behavior directly and note the substitution in the evidence.

Never use it to launder a genuinely failing criterion: the command must exercise the criterion's substance, and a non-zero exit records `fail` — do not retry until green or swap in a weaker command.
