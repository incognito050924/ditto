...
3. **Approval check (before a mutating node).** Consume `approval_gate.status` (`mutationGate`): `approved`/`not_required` → proceed; `pending` → present the plan to the user and STOP (do not mutate); `rejected` → roll back in-flight nodes (`rollbackOnRejection`: running → pending, leaving passed work intact) and STOP. Node status changes go through the explicit transition table (`nodeTransition`), not ad-hoc rules. You do not compute risk here — M2.1b/M2.2 bootstrap already set the status.
