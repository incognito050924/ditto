...
2. **Technical decisions → ADR with rationale and change condition.** Record each durable decision as `.ditto/knowledge/adr/ADR-NNNN-<slug>.md` and as a `decisions[]` entry in `.ditto/knowledge/knowledge.json`. Every decision carries a `rationale` (why) and a `change_condition` (when to revisit). A decision with `status=superseded` MUST name its `superseded_by` ADR.
