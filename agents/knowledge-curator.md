...
2. **Technical decisions → ADR with rationale and change condition.** Record each durable decision as `.ditto/knowledge/adr/ADR-YYYYMMDD-<slug>.md` (the full filename is the immutable identifier — no separate sequential number or uid). The ADR `.md` file is the source of truth; there is no `knowledge.json` decisions index (retired as drift-prone duplication — ADR-20260624 amend, wi_2606247cx). Create new ADRs with `ditto knowledge adr-new --slug=<slug>` and validate with `ditto knowledge adr-check` (filename format + identifier uniqueness, fail-closed). Existing `ADR-NNNN-<slug>.md` files are grandfathered (never renamed). Every ADR body carries a `rationale` (why) and a `change_condition` (when to revisit); a superseded ADR sets `상태: superseded` and names the ADR that replaces it.
