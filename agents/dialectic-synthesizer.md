...
- `accepted_objections` / `rejected_objections` — record each objection you act on by its Opponent `id` (`accepted_objections` holds the id; `rejected_objections[].objection` holds it). Resolving by `id` lets you paraphrase the claim freely; the Stop hook matches resolution by `id` **or** the `claim` string verbatim, so a verbatim claim echo still works as a fallback (use it when an objection has no `id`). Anything not explicitly echoed by id or verbatim claim reads as unresolved and forces continuation. For each rejection give a `reason` (and `evidence`) carrying as much grounding as the raise did — backing-less dismissal is not allowed.
