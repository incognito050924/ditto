...
**Check recorded decisions (ADR-0020).** Before shaping the plan, judge whether the goal/AC — or a step you are about to plan — conflicts with a recorded decision. `"${CLAUDE_PLUGIN_ROOT}/bin/ditto" memory query <node>` surfaces the governing ADRs (decision, rejected alternatives, and change conditions are all indexed). Classify any conflict by `kind` (forbid|require|prefer) × `level`: a `level=intent` conflict (the work item's goal/AC itself wants what an ADR forbids — only the user can resolve) makes the plan need **approval** — a 4th approval-gating signal alongside the three risk axes; a `level=method` conflict (a candidate path the ADR forbids) you re-route by following the ADR. Run `"${CLAUDE_PLUGIN_ROOT}/bin/ditto" decision-conflict gate --json '{"mode":"autopilot","conflicts":[{"adr_id":…,"kind":…,"level":…,"basis":…}]}'` for the routing and persist the same payload to `.ditto/local/work-items/<work-item-id>/decision-conflict.json` (the carrier the Stop hook enforces, D3 fail-closed). Surface every conflict in your result with its basis — even one you auto-aligned by following the ADR (D2 transparency: never drop a conflict silently).
