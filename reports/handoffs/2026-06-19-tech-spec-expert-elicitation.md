...
tech-spec 작성 시 에이전트가 **과제별 전문가 고려사항을 "좋은 질문"으로 유도**(맹점을 보게 + 다른 시각으로 사고 확장 + 방향 유지)하고, 답 못하는 사용자를 **편향 비용 순 도움 사다리**로 돕고, 사용자 답변과 산출물 전체에 **상시 비판축(pre-mortem)**을 돌리며, **질문 문구는 산출물에 누출하지 않는다.** 목표: 누가 쓰든 산출물 품질 균일화. deep-interview는 모호성 축소 축으로 **무변경**.
