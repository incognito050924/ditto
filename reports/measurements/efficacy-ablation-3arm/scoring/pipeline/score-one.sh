...
sdir=$SCORING/sessions/attempt-$A
sbclone=$SB/attempt-$A/work/palimpsest
scratch=$SCRATCH_BASE/attempt-$A
...
blind_exit=0
(cd "$HARNESS" && ./score.sh blind --session "$sdir") > "$sdir/blind-scan.out" 2>&1 || blind_exit=$?
...
score_exit=0
(cd "$HARNESS" && ABLATION_PYTEST_CMD="$PYTEST_CMD" ./score.sh score --session "$sdir") > "$sdir/score-run.out" 2>&1 || score_exit=$?
