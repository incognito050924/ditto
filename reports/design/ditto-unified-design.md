...
**남은 구현 gap (조사·테스트로 확인):**
- **⑥ Senior Pattern Repository** — `architecture propose`는 비권위 후보만 생성. 관찰→ratification→ADR 저장소 자동화 미구현(~30%).
- **CodeQL WI-4(Opponent)/WI-5(Dataflow DoD)** — 미착수(WI-4는 무한루프 방지 조건 충족 후 차단 전환; WI-5는 PoC만).
- **Multi-Change Semantic** — 여러 signature 동시 변경 시 하나만 자동 감지(MVP는 다중 시 fail-closed).
- **`fitness` executed-mode 실행 provider** — 컴파일 언어 clean build 실행 provider·Assurance drift 집계 뷰 잔여(ADR-0004 Q4).
- **평가 지표 수집** — §5의 ground truth 지표를 *정의*했으나 실제 측정·집계 파이프라인은 없다.
- **축4 durable 판정의 휴리스틱 의존** — "가치 있는 변경" 판정이 에이전트 휴리스틱에 머물러, under-recording·over-recording을 둘 다 줄이는 게이트가 미구현.
