...
- **동작(control-flow)**: 게이트 함수들을 순수 TS로 구현 — `deterministicFloor(ledger)`(열린 필수 섹션·CONFLICTING·assumption 비율 산술 → ambiguity 하한), `acceptanceTestable(ac)`(VAGUE_TERMS 상수 + observable 정규식), `completionGate(workItem, completion)`(final_verdict=pass ⇒ 모든 acceptance verdict=pass **∧ `completion.acceptance[].criterion_id` 집합 == `workItem.acceptance_criteria[].id` 집합** — 누락·잉여·중복 모두 FAIL), `convergenceGate(convergence)`(두 게이트 교집합 + admissibility), `highRiskAssumption(assumption)`(non-local ∨ irreversible ∨ unaudited 중 하나라도 참이면 high-risk → M2.3 approval gate 자동 트리거 신호 반환; 셋 다 거짓이면 safe). 각 게이트는 fixture를 입력받아 PASS/FAIL + 사유 반환. LLM 호출 없음(D5).
...
- **v0 범위 한정(승인 채널 미해결 — §8-1)**: 사용자 입력을 **블로킹 대기**하는 승인 채널 메커니즘(PreToolUse 권한 프롬프트 활용 여부 등)은 §8-1 미해결 선결조건이다. v0 M2.3은 그 채널을 *구현하지 않고*, **이미 set된 `approval_gate.status`만 소비**한다 — source는 `approved_spec`/`issue`/`prd`/`small_reversible_policy` 또는 직전 turn의 명시 승인(autopilot 상세 §5.2). 따라서 v0 M2.3 = "현재 status를 읽어 `pending`이면 mutation 중단 + plan 제시, `approved`/`not_required`면 무중단 진행". 인터랙티브 블로킹 채널은 §8-1 확정 후 별도 build unit으로 승격(§7 빌드그래프 `[§8-1]` 엣지).
- **대상 계약/스키마**: §5.4, autopilot 상세 §5.
- **acceptance**: **미리 `pending`으로 set된 fixture** → mutation 노드 미실행 + plan artifact 생성. **`approved`/`not_required` fixture** → 무중단 진행. (블로킹 입력 대기 동작은 v0 acceptance에 포함하지 않음.)
...
- **동작(control-flow)**: 노드 passed → 다음 ready 노드로 자동 진행(개입 없음). 내부 checkpoint 완료를 final answer로 보내지 않음. context pressure 시 `/handoff`로 같은 autopilot_id 유지하며 이어받음(scope 축소 아님). stateless 노드(단일 결과 반환, 부모 상태 누수 방지).
...
3. **autopilot continuation 재진입 경계** — 같은 세션 내인지 새 세션(handoff 재로드)인지, 상태 복원 절차. M2.5 설계 시 확정.
4. **safe-default 정책 미명세 → M0.4 `highRiskAssumption`과 동일 정책으로 통일(해소).** ouroboros `finalize_safe_defaultable_gaps`의 "local·reversible·audited"는 M0.4 `highRiskAssumption`의 부정과 **정확히 같은 3축**이다: `safe = ¬(non-local ∨ irreversible ∨ unaudited) = local ∧ reversible ∧ audited`. 따라서 별도 정책을 새로 정의하지 않고 **`highRiskAssumption(x)==safe`를 곧 "safe-defaultable"의 단일 정의로 삼는다**(둘은 한 술어의 양면). 이 술어가 거짓(=high-risk)이면 §8-4 기본대로 fail-closed(M2.3 approval gate 트리거). — *판정 위치*만 주의: safe-default 적용은 gate(glue), 정책 술어는 M0.4 1곳.
