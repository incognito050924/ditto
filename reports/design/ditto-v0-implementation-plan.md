...
- **M3 Evidence·verifier 런타임**: PostToolUse evidence 수집, verifier output contract, completion.json 생성, convergence gate 런타임(admissibility·ratchet·decision ledger). v0의 계약/스키마/게이트(M0)에 *런타임 살*을 붙임. **wi_v04verifier_body_and_declared_by (2026-06-01)**: `CompletionContract.declared_by`를 자유 문자열에서 `declarerRole` enum(`main|planner|implementer|verifier|reviewer|researcher|synthesizer`)으로 좁혀 *판정 주체*(설계서 line 700)를 실행 프로파일(`profileName`)과 분리 — implementer가 verifier를 사칭하거나 실행 프로파일 문자열(`workspace-write`)을 declarer로 박는 것을 schema 단에서 reject. `ditto work handoff --declared-by`(default `main`)로 main이 자기 완료를 선언하는 path 명시. `declared_by`를 읽어 권한 결정하는 소비처 없음 → permission 무영향. **wi_v04evidence_record_sidecar (2026-06-01)**: 설계서 §6.7 line 633-645의 `EvidenceRecord` sidecar(`evidenceRef` 재사용 + `captured_at`/`freshness`/`stale_reason`/`portability`/`artifact_available`/`exit_code`/`key_lines`) + cross-field(stale⇒reason / fresh⇒null / committed⇒available)를 `src/schemas/evidence-record.ts`로 구현. 커밋 가능 ledger `evidence-index.json`(work-item 루트, `EvidenceStore.appendRecord`/`readIndex`)로 "증거 존재"와 "이 clone/세션에서 raw 열람 가능"을 분리 — raw `evidence/`는 계속 gitignore. `CompletionContract.acceptance[].evidence_records`(optional, default [])로 freshness 래핑 evidence를 선택적으로 실음(기존 `evidence` bare ref 폐기 X, 마이그레이션 불필요). **wi_v04dialectic_runtime (2026-06-01)**: dialectic 3역(producer/opponent/synthesizer) agent 본문 + `/ditto:dialectic` skill driver 절차 + `OpponentModelRouter`(`src/core/opponent-router.ts` — codex→claude-opus→claude-sonnet 후보 해석 + fallback provenance 기록, 실제 CLI glue는 별도 §3.4) + Stop hook의 `reviews/dialectic-*.json` admissibility cross-check(verdict reject/blocked·미해결 admissible objection→continuation, taste 제외, malformed fail-closed). **결정**: 권위 문서(autopilot-contract §2.2, dialectic-contract §1.2)상 dialectic은 새 nodeKind가 아니라 review/high-impact 노드의 검증 메커니즘 → nodeKind/owner enum 미변경.
