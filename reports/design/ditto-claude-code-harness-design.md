...
- **하나의 의도는 하나의 완료 단위다.** 사용자가 지시한 범위가 크다는 것은 최초 의도 자체가 크다는 뜻이므로, agent는 한 요청을 임의로 여러 work item으로 쪼개거나 범위를 축소하지 않고 하나의 work item·오케스트레이션으로 끝까지 완수한다(하나의 요청 = 하나의 work item). 계획, 구현, 리뷰, 수정, E2E, 문서, knowledge update는 내부 checkpoint일 뿐 사용자에게 매 phase마다 넘기는 완료 단위가 아니다. 충돌·광범위 같은 정당한 분리 사유가 있으면 agent가 분리를 제안하되 최종 결정은 사용자가 하며, 분리는 끝까지 완수를 회피하는 수단이 아니다(분리 후 남은 범위도 끝까지 완수하고, 분리분은 버리지 않고 별도로 추적한다).
...
| 큰 의도를 임의로 축소·분할하지 않고 끝까지 완수 | `OneShotOrchestrationContract`가 계획-구현-검증-수정-완료를 하나의 의도 안에서 끝까지 소유한다. 범위 분리는 정당한 사유가 있을 때 agent가 제안하고 사용자가 결정하되, 완수 회피 수단이 되지 않는다. |
...
- graph 드라이버는 main agent가 `orchestrate` skill로 구동한다(§6.5, §7.3, one-shot §3.4). 사용자에겐 현재 목적과 최종 결과만 노출하고, 노드 선택·실패 분류 판단은 `orchestration.json`에 남긴다.
...
- v0 오케스트레이션은 일반 delegation(main→stage subagent 단방향)을 그대로 쓴다 — orchestrator가 main agent role이라 별도 양방향 계약이 없다. (결정 로직을 결정자 subagent로 떼는 future 옵션에서만 orchestrator↔main 양방향 계약이 필요 — one-shot §3.5.)

### 6.5 One-Shot Orchestration Contract

> **상세 설계.** 이 절은 "what"(목적·불변규칙·범위 보존)만 둔다. 오케스트레이션 그래프 구조, 드라이버(`orchestrator`)의 ReAct 루프, 실패 분류·결정, approval gate 메커니즘, `orchestration.json` 스키마 등 "how"는 [`contracts/one-shot-orchestration-contract.md`](contracts/one-shot-orchestration-contract.md)가 소유한다. 충돌 시 권위는 실제 스키마 > 본 절(what) > 상세문서(how) 순이다(§3.3 일반화).
...
- 그래프 드라이버는 main agent가 `orchestrate` skill로 구동한다(§7.3, 전용 subagent 아님 — one-shot §3.4). content 생성 없이 노드 선택·stage subagent spawn·실패 분류·정지 평가만.
...
- 노드 실패는 orchestration 실패가 아니라 분류 입력이다. retry/switch는 자동, escalate·user-owned decision만 사용자에게 간다. cap 도달은 성공이 아니다(`cap-reached ≠ converged`, §6.9).
- blocker는 "진행할까요?"가 아니라 어떤 user-owned decision이 필요한지로 묻는다(§6.2).
- context handoff가 필요해도 orchestration은 같은 `orchestration_id`로 이어진다(§6.10). "이번 turn에 다 못했다"는 이유로 scope를 줄이지 않는다.
- 큰 작업일수록 subagent-first(§6.4)·verifier-first를 강하게 적용한다.

범위 보존과 정당한 분리:

- 사용자가 지시한 범위가 크다는 것은 최초 의도 자체가 크다는 뜻이다. agent는 한 요청을 임의로 여러 work item으로 계획·분산하지 않고 하나의 work item·orchestration으로 끝까지 완수한다(하나의 요청 = 하나의 work item, 임의 분산·축소 금지). 이는 §1의 "최초 의도 확대 금지"와 대칭이다.
- 다음의 경우에 한해 agent는 범위 *분리를 제안*할 수 있고, 분리 여부의 최종 결정은 사용자가 한다(user-owned decision → `blocked_by_user_owned_decision`, §5.4 plan approval gate에서 함께 제시).
  - **충돌 분리**: 변경 간 충돌·오염 위험이 클 때. 예: 엔티티/DB 구조 변경과 그에 의존하는 기능·버그 수정의 분리, 기능 개발에 선행해야 하는 대규모 구조적 변경(Tidy First)의 분리. 단 commit 단위의 구조적/동작적 분리는 한 orchestration 내부의 commit discipline이지 범위 분리가 아니다.
  - **광범위 분리**: 연관성 없는 다수 기능을 한 번에 다룰 때. 예: 서로 무관한 여러 메뉴 동시 개발, 권한 관리 기능 자체와 서비스·리소스에 대한 권한 적용을 한 번에 개발.
- 분리는 끝까지 완수를 회피하는 수단이 아니다. 분리가 사용자 승인으로 확정되면 (a) 현재 orchestration에 남은 범위는 여전히 끝까지 완수하고, (b) 분리분은 버리지 않고 sibling follow-up work item 또는 `out_of_scope` + `follow_up_candidates`로 남겨 결정 근거와 함께 추적한다(§6.1, §6.8). v0에서는 child work item을 만들지 않으므로(위 규칙), 분리분은 현재 orchestration의 하위가 아니라 별도(후속) work item이다.
...
사용 시점(요약): 중요한 설계문서·제안서·PRD·ADR 작성/개정, 기존 산출물 리뷰(승인/수정/기각), 기술·제품 결정 대안 검토, recommendation 생성, 큰 one-shot orchestration의 plan/checkpoint 검증, final answer 전 high-impact 검증.
...
skills/
  ...
  handoff/SKILL.md
  orchestrate/SKILL.md        # one-shot 드라이버 (main agent 구동, 비노출)
agents/
  researcher.md
  planner.md
  implementer.md
...
- `/ditto:plan`은 v0 skill이지만 주로 one-shot orchestration이 내부 호출한다. §9의 노출 command 목록에 의도적으로 넣지 않으며(비노출), 필요 시 직접 호출만 가능하다.
...
- 오케스트레이션은 전용 subagent가 아니라 **main agent가 `orchestrate` skill로 구동**한다(§7.3, one-shot §3.4) — subagent는 subagent를 spawn 못 하므로 spawn 루프는 main에 산다. main이 stage subagent를 1-레벨로 spawn하고 Stop hook이 루프를 유지하며, orchestrate skill은 content 생성 없이 노드 선택·실패 분류·정지 판단만 한다("orchestrator, not implementer"). 그래프 *상태*는 `orchestration.json`(schema), *판단*은 skill 프롬프트(§11 원칙 10), 루프 *지속*은 Stop hook — 세 축을 섞지 않는다. 주류(Sisyphus=primary, OMC Ralph+persistent Stop) 일치.
- 결정 로직을 별도 결정자 subagent로 떼는 분리(HANNES harness 패턴)는 v0 후 결정 context 오염 증거 시의 future refinement다(one-shot §3.5) — v0엔 과잉.
...
| `/ditto:plan` | skill skeleton (one-shot orchestration 내부 호출, 비노출) | M2 |
...
- `orchestrate` skill skeleton (main agent 구동: ready 노드 선택 · stage subagent spawn · 실패 분류, §6.5)
...
- main agent가 `orchestrate` skill로 ready 노드를 골라 stage subagent를 spawn하고, 노드 실패를 분류해 retry/switch/escalate를 결정한다.
