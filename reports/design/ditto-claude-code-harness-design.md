...
목적: 반복 정련(review→edit, verify→fix, dialectic 라운드)이 **treadmill(같은 지점 진동·점진적 품질 저하)에 빠지지도, 조기 수렴(명백한 이슈 무시·no-op 답변·정해진 횟수만 돌고 종료)에 빠지지도** 않게 한다. 이는 §6.8 Completion Contract의 쌍대(dual)이며, 제1원칙의 직접 파생이다.
...
- `done`은 **카운트나 "더 할 말 없는 느낌"이 아니라** 두 게이트 동시 만족으로만 정의된다: *모든 AC가 증거로 닫힘 + admissible한 열린 반론 0개*.
- 라운드 1에 이게 참이면 라운드 1에 멈춘다. 라운드 N에도 거짓이면 끝난 게 아니라 non-pass로 닫고, 최종 verdict(`partial|fail|unverified`)와 work item status는 CompletionContract(§6.8)가 결정한다.

admissibility gate (반론이 *행동*을 트리거할 자격):

- 반론은 (a) 구체적 반례를 인스턴스화하거나 위반하는 acceptance criterion에 매핑되고(oracle), (b) decision ledger에 없던 신규이며(memory), (c) critical/major severity일 때만 admissible하다.
- **대칭 원칙**: 반론을 *기각*하는 데도 raise만큼의 이유+근거가 든다. 무료 묵살 금지.
- **묵살 불가 원칙**: acceptance criterion에 매핑되는 반론은 절대 inadmissible로 선언할 수 없다. criteria가 바닥이다. 게이트는 취향·criterion 없는 반론만 거른다.

ratchet (품질 단조 비퇴행):

- **최선본 보존**: 모든 버전을 불변 기준으로 점수 매기고, 산출물은 "마지막 버전"이 아니라 "최고 점수 버전"으로 한다. degradation이 정의상 불가능해진다.
- **결정 원장**: 채택/기각한 모든 반론을 이유+근거와 함께 기록하고 다음 라운드에 구속력을 준다. 이미 기각된 지점을 새 근거 없이 재제기하면 자동 각하 → A→B→A 진동이 불가능해진다.
- **라운드 캡은 안전 정지지 성공 조건이 아니다.** 캡 도달 시 두 게이트를 만족 못 했으면 그건 done이 아니라 non-pass(verdict는 CompletionContract가 `partial|fail|unverified`로 결정) + handoff다. `cap-reached ≠ converged`.

정직 라벨 (제1원칙의 투명성 구현):

- 모든 산출물 항목에 세 라벨을 붙인다: *정체*(`finding|hypothesis|taste`), *확신*(근거/oracle 출처, 백킹 없으면 낮음 명시), *상태*(`acted|deferred|dismissed`+이유).
- 백킹 없는 주장은 `finding`이 아니라 `hypothesis`로 라벨링하고 행동하지 않는다.
- 투명성은 계층적이다: 숨기는 것은 없되 actionable한 것을 전면에 두고 나머지는 접거나 on-demand로 둔다. "모든 이슈를 같은 볼륨으로 쏟기"는 정직이 아니라 노이즈 복원이다.
...
`convergence.json`은 버전별 점수, decision ledger, deferred/dismissed 항목과 이유를 담는 additive sidecar다. `completion.json`(§6.8)을 대체하지 않는다.
