...
- 라운드 1에 이게 참이면 라운드 1에 멈춘다. 라운드 N에도 거짓이면 끝난 게 아니라 `blocked|partial`이다.
...
- **라운드 캡은 안전 정지지 성공 조건이 아니다.** 캡 도달 시 두 게이트를 만족 못 했으면 그건 done이 아니라 `blocked|partial` + handoff다. `cap-reached ≠ converged`.
...
1~9, 12, 13, 14는 v0(Milestone 0~2) 수락 기준이다. 10~11은 post-v0(Milestone 5~6) 수락 기준으로, v0 완료 판정에는 포함하지 않는다.
