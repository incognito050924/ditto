...
> **제1원칙 (prime directive).** DITTO는 **근거가 있는 것에만 *행동*하고, 모든 이슈는 *드러내며*, 백킹 없는 것은 *주장하지 않는다*.** 핵심은 **행동의 바와 드러내기의 바가 다르다**는 것이다 — 드러내기(surface)는 낮은 바라서 취향·가설·sub-threshold 우려도 숨기지 않고 기록하고, 행동(편집·차단·계속·done 선언)은 높은 바라서 증거와 acceptance criterion에 연결된 것만 통과한다. 그 사이 간격은 *버려지지 않고 원장에 보인다*. 이 원칙이 §6의 쌍대 게이트(Completion ↔ Convergence)와 evidence/admissibility 규칙 전부의 부모다. LLM은 입력에 대해 무엇이든 생성하므로, DITTO의 목표는 모델이 트집·할루시네이션을 *못 내게* 하는 게 아니라, 백킹 없는 생성을 *행동으로 전환되기 전에 무력하게* 만드는 것이다.

> **멘탈 모델 (한 줄).** DITTO는 자율주행 *시스템*이고, Claude Code/Codex는 *자동차*다. 우리는 자동차(모델 루프·도구·permission·sandbox = 엔진·구동계·액추에이터)를 만들지 않는다. 자율주행 시스템(의도 파악·경로 계획·오케스트레이션·완료/수렴 판단)을 만들어 자동차에 이식(통합)한다. 자율주행 시스템에 운전 주체(orchestrator)가 있는 것은 당연하며 그것이 자동차를 다시 만드는 게 아니다 — §2 비목표 #1, §7.4 orchestrator, §11 원칙 10(액추에이터는 결정론, 어디로 갈지는 판단)의 공통 근거다.
...
- **v0는 작게 시작한다.** Milestone 0~2의 contract/plugin/autopilot skeleton까지만 v0로 묶고, evidence hardening, context rot 방지, E2E, project-management integration은 후속으로 분리한다.
- **하나의 의도는 하나의 완료 단위다.** 사용자가 지시한 범위가 크다는 것은 최초 의도 자체가 크다는 뜻이므로, agent는 한 요청을 임의로 여러 work item으로 쪼개거나 범위를 축소하지 않고 하나의 work item·autopilot으로 끝까지 완수한다(하나의 요청 = 하나의 work item). 계획, 구현, 리뷰, 수정, E2E, 문서, knowledge update는 내부 checkpoint일 뿐 사용자에게 매 phase마다 넘기는 완료 단위가 아니다. 충돌·광범위 같은 정당한 분리 사유가 있으면 agent가 분리를 제안하되 최종 결정은 사용자가 하며, 분리는 끝까지 완수를 회피하는 수단이 아니다(분리 후 남은 범위도 끝까지 완수하고, 분리분은 버리지 않고 별도로 추적한다).
...
DITTO
  ...
  autopilot / completion contract / handoff
...
| 큰 의도를 임의로 축소·분할하지 않고 끝까지 완수 | `AutopilotContract`가 계획-구현-검증-수정-완료를 하나의 의도 안에서 끝까지 소유한다. 범위 분리는 정당한 사유가 있을 때 agent가 제안하고 사용자가 결정하되, 완수 회피 수단이 되지 않는다. |
...
> **비목표 #1 명확화 (orchestration ≠ loop 재구현).** #1이 금지하는 것은 모델↔도구 turn loop 자체를 다시 만들거나 provider CLI를 raw로 조립하는 것이다. 여러 전용 에이전트를 native Task spawn으로 조율해 하나의 의도를 끝까지 모는 **오케스트레이션 층은 재구현이 아니라 기능 추가**이며 DITTO가 소유한다(§5.3, §6.5, §7.4). 즉 DITTO는 provider의 agent loop를 다시 짓지 않고 그 *위에* 오케스트레이션을 얹는다. HANNES `harness`·OMX Sisyphus가 이 층의 선례다.
...
- `AutopilotContract`: 큰 범위 작업을 내부 단계로 나누되 하나의 실행 의도로 끝까지 완수하는 계약
...
- `ConvergenceContract`: 반복 정련의 treadmill/조기수렴 방지. CompletionContract의 쌍대 게이트
...
| work item status | `draft`, `in_progress`, `blocked`, `partial`, `unverified`, `done`, `abandoned` | 새 status enum을 만들지 않는다. 내부 interview/planning/running/verifying 상태는 `autopilot.json` checkpoint 또는 `progress.md`에만 둔다. |
...
| autopilot | 없음 | `autopilot.json`은 additive sidecar다. work item status를 대체하지 않고 checkpoint graph만 저장한다. |
...
### 4.3 기층 4축의 배포 — 횡단축 Distribution + session-rooting invariant

§4.1의 기층 4축(Hooks/Skills/Agents/State)은 "런타임 **위에서 어떻게 동작하는가**"를 다루지만 "어떻게 타겟에 **올라가 살아있게 되는가**"(배포)는 다루지 않는다. boxwood 실타겟 실증(`ditto-install-distribution-record.md` §2·§6)이 그 빈자리를 드러냈고, 본 절이 그것을 canonical로 박는다. 결정 근거·대안·변경 조건은 **ADR-0011**에 있다.

**(A) Distribution = 기층 4축에 직교하는 횡단 배포계약 축.** 기층 4축의 5번째 멤버가 아니라 네 축 각각에 직교로 걸리는 관심사다 — 각 기층 축은 충족해야 할 **배포 계약(deployment contract)** 을 갖는다.
...
- **구현체**: `scripts/install-plugin.mjs`의 단계(register/build/place/init/allowlist).
- **점검 표면**: install `status`가 방출하는 플래그는 `plugin_enabled / binary_built / binary_on_path / codeql / playwright / target_initialized / allowlisted`다. `ditto doctor distribution`(`src/core/distribution-doctor.ts`)이 배포계약 충족 여부를 4 기층축으로 매핑하며, `hooks_registered`는 install status가 아니라 doctor가 추가로 점검하는 항목, `allowlisted`는 보고용(어느 축 게이트에도 안 들어감)이다. **알려진 한계**: doctor가 plugin-root와 target-root를 같은 `repoRoot`로 점검하므로 self-host/병치 레이아웃에서만 정확하다(타겟-루트 세션에선 plugin 아티팩트 오판 가능 — 4e 후속). 상세는 ADR-0011 D1.

**(B) Session-rooting invariant (cross-repo subagent 위임 비지원).**

> **DITTO의 cross-repo subagent 쓰기는 세션 repoRoot 밖이면 PreToolUse scope-out에 차단되고, 충실한(subagent 위임) autopilot 루프는 타겟 레포에 루트된 세션을 요구한다.**

- scope-out이 차단하는 것은 **세션 repoRoot 밖 쓰기**다(읽기·CLI는 대상 아님). boxwood inc6도 cross-repo subagent **쓰기**가 막힌 것이고, main agent가 노드 owner를 대행하면 cross-repo에서도 autopilot이 **완주**했다(§2.6·§6).
- **운영 함의**: 타겟을 (subagent 위임까지) autopilot으로 돌리려면 **그 타겟에 루트된 세션을 띄운다**. 다른 레포에서의 원격 subagent 오케스트레이션은 비지원.
- scope-out 차단은 **버그가 아니라 경계**다(repo 경계 보호 설계의도와 정합).
- **층위 구분**: 이 cross-repo(런타임 기층 세션 루팅)는 ADR-0007의 cross_repo(정적분석 CodeQL ImpactGraph의 형제모듈 타입참조)와 **다른 층위**다 — `pre-tool-use.ts`에서도 scope-out 쓰기 가드 vs JVM cross_repo 가드로 별도 분기다(ADR-0011 D3).
...
### 5.2 Autopilot large work flow

큰 범위 작업은 사용자에게 "phase 1만 완료했습니다"라고 반환하는 방식으로 처리하지 않는다. DITTO는 큰 작업을 내부적으로 나누지만, 외부 완료 단위는 하나의 work item이다. 이것은 §0 제1원칙의 적용이다: 진척에 대한 *행동(계속)*의 바는 stop condition이지 "한 단계 끝남"이 아니다.
...
- agent는 기본적으로 계속 진행한다. 멈춤은 stop condition(§6.5)에 해당할 때만 일어나는 예외이며, "다음 단계를 진행할지"는 사용자에게 묻지 않는다.
...
- 실패가 나면 같은 autopilot 안에서 diagnose -> fix -> verify를 반복한다.
- 전체 acceptance criteria가 pass/partial/fail/unverified로 닫히기 전에는 완료라고 말하지 않는다.

### 5.3 Autopilot graph

autopilot은 linear phase list가 아니라 작업 그래프다. 그래프는 agent가 내부적으로 관리하고, 사용자에게는 현재 목적과 최종 결과만 노출한다.
...
- node 실패는 autopilot 실패가 아니라 수정 루프 입력이다.
...
- graph mutation은 `AutopilotStore`를 통해서만 한다.
- graph 드라이버는 main agent가 `autopilot` skill로 구동한다(§6.5, §7.3, autopilot §3.4). 사용자에겐 현재 목적과 최종 결과만 노출하고, 노드 선택·실패 분류 판단은 `autopilot.json`에 남긴다.
- final report는 graph 전체 상태와 acceptance criteria 상태를 기준으로 한다.

### 5.4 Plan approval gate

autopilot은 "승인 없는 폭주"가 아니다. DITTO는 승인 경계와 지속 실행 경계를 분리한다.
...
- 내부 상태인 interviewing/planning/running/verifying은 `work-item.json` status가 아니라 `autopilot.json` checkpoint와 `progress.md`에 기록한다.
...
- 대칭으로, agent가 임의로 목표를 축소하거나 한 요청을 여러 work item으로 분산해서도 안 된다. 하나의 요청은 하나의 work item이 원칙이며, 분산은 §6.5의 정당한 분리 기준에 매핑되고 사용자가 결정할 때만 허용된다. 이는 큰 범위 작업뿐 아니라 모든 요청에 적용된다.
...
> **상세 설계.** 이 절은 "what"만 둔다. 질문 생성 메커니즘, 모호성 수치화 구조, 종료 게이트, 사이드카 스키마 등 "how"는 [`contracts/deep-interview-contract.md`](contracts/deep-interview-contract.md)가 소유한다. 충돌 시 권위는 실제 스키마 > 본 절(what) > 상세문서(how) 순이다(§3.3 일반화). 이 분리는 per-contract 상세 문서의 첫 사례다.

목적: 사용자도 인지하지 못한 의도의 모호성을 **변증법적(소크라테스식)·무편향 질문**으로 필요한 만큼만 채우고, **pre-mortem**으로 되돌리기 어려운 위험을 조기에 드러낸다. 이는 계획·현황파악 단계의 사전 게이트이며, 초기 의도 정렬 비용이 사후 수정 비용보다 싸다는 가설에 기반한다. intent 층위의 모호성 축소만 다루며, plan 층위 적대 검증은 §6.6이 맡는다(omx `deep-interview` → `ralplan` 분리와 동형).
...
- 질문은 한 번에 하나씩 하되, 후보는 모든 모호성 차원에서 생성해 앵커링을 막는다(무편향).
- 각 질문은 §6.2 QuestionGate self-answer를 먼저 통과하고, "답에 따라 무엇이 달라지는지"를 포함한다.
- 사용자가 답하지 않아도 되는 구현 세부사항은 agent가 판단한다.
- 종료는 모호성 게이트(critical 차원 해소 AND readiness 임계 통과)로 정의한다. 질문 상한 도달은 성공이 아니며, 남은 모호성은 명시적 가정(`hypothesis` 라벨, §6.9)으로 기록한다.
...
- v0 autopilot은 일반 delegation(main→stage subagent 단방향)을 그대로 쓴다 — orchestrator가 main agent role이라 별도 양방향 계약이 없다. (결정 로직을 결정자 subagent로 떼는 future 옵션에서만 orchestrator↔main 양방향 계약이 필요 — autopilot §3.5.)

### 6.5 Autopilot Contract

> **상세 설계.** 이 절은 "what"(목적·불변규칙·범위 보존)만 둔다. autopilot 그래프 구조, 드라이버(`orchestrator`)의 ReAct 루프, 실패 분류·결정, approval gate 메커니즘, `autopilot.json` 스키마 등 "how"는 [`contracts/autopilot-contract.md`](contracts/autopilot-contract.md)가 소유한다. 충돌 시 권위는 실제 스키마 > 본 절(what) > 상세문서(how) 순이다(§3.3 일반화).

목적: 큰 범위 작업을 "한 phase씩 처리"하지 않고 하나의 사용자 의도 안에서 끝까지 완수한다. 사용자가 지시한 범위는 임의로 축소·분할하지 않는다. 내부 분할은 드라이버(`orchestrator`, §7.4)의 스케줄링 책임이며 사용자-facing 완료·승인 경계가 아니다.

저장 위치:

```text
.ditto/work-items/<id>/autopilot.json
.ditto/work-items/<id>/autopilot.md
```

핵심 불변규칙(what — 그래프·드라이버 루프·실패분류·스키마는 상세문서가 소유):

- v0에서는 autopilot 1개가 work item 1개에 대응한다. child work item을 만들지 않으며 `completion_boundary`는 그 단일 work item이다.
- 그래프 드라이버는 main agent가 `autopilot` skill로 구동한다(§7.3, 전용 subagent 아님 — autopilot §3.4). content 생성 없이 노드 선택·stage subagent spawn·실패 분류·정지 평가만.
- 내부 checkpoint(노드) 완료는 보고할 수 있지만 final answer의 근거일 뿐 final answer 자체가 아니다.
- 큰 범위 mutation은 approval gate 통과 후 시작하고, 통과 뒤에는 checkpoint 사이에서 재승인을 기다리지 않는다(§5.4).
- 노드 실패는 autopilot 실패가 아니라 분류 입력이다. retry/switch는 자동, escalate·user-owned decision만 사용자에게 간다. cap 도달은 성공이 아니다(`cap-reached ≠ converged`, §6.9).
- blocker는 "진행할까요?"가 아니라 어떤 user-owned decision이 필요한지로 묻는다(§6.2).
- context handoff가 필요해도 autopilot은 같은 `autopilot_id`로 이어진다(§6.10). "이번 turn에 다 못했다"는 이유로 scope를 줄이지 않는다.
- 큰 작업일수록 subagent-first(§6.4)·verifier-first를 강하게 적용한다.

범위 보존과 정당한 분리:

- 사용자가 지시한 범위가 크다는 것은 최초 의도 자체가 크다는 뜻이다. agent는 한 요청을 임의로 여러 work item으로 계획·분산하지 않고 하나의 work item·autopilot으로 끝까지 완수한다(하나의 요청 = 하나의 work item, 임의 분산·축소 금지). 이는 §1의 "최초 의도 확대 금지"와 대칭이다.
- 다음의 경우에 한해 agent는 범위 *분리를 제안*할 수 있고, 분리 여부의 최종 결정은 사용자가 한다(user-owned decision → `blocked_by_user_owned_decision`, §5.4 plan approval gate에서 함께 제시).
  - **충돌 분리**: 변경 간 충돌·오염 위험이 클 때. 예: 엔티티/DB 구조 변경과 그에 의존하는 기능·버그 수정의 분리, 기능 개발에 선행해야 하는 대규모 구조적 변경(Tidy First)의 분리. 단 commit 단위의 구조적/동작적 분리는 한 autopilot 내부의 commit discipline이지 범위 분리가 아니다.
  - **광범위 분리**: 연관성 없는 다수 기능을 한 번에 다룰 때. 예: 서로 무관한 여러 메뉴 동시 개발, 권한 관리 기능 자체와 서비스·리소스에 대한 권한 적용을 한 번에 개발.
- 분리는 끝까지 완수를 회피하는 수단이 아니다. 분리가 사용자 승인으로 확정되면 (a) 현재 autopilot에 남은 범위는 여전히 끝까지 완수하고, (b) 분리분은 버리지 않고 sibling follow-up work item 또는 `out_of_scope` + `follow_up_candidates`로 남겨 결정 근거와 함께 추적한다(§6.1, §6.8). v0에서는 child work item을 만들지 않으므로(위 규칙), 분리분은 현재 autopilot의 하위가 아니라 별도(후속) work item이다.
- 분리 제안의 바는 높다. "작업이 커 보인다"는 느낌만으로 분리하지 않으며, 위 두 사유 중 하나에 구체적으로 매핑될 때만 제안한다. 이 "사용자 승인에 의한 분리"는 "이번 turn에 다 못해서 줄이는" 임의 축소(§5.2)와 구분되며, 전자는 결정 근거가 남고 후자는 금지된다.
...
> **상세 설계.** 이 절은 "what"(목적·3역 책임·불변규칙)만 둔다. 3역 분리 메커니즘, mode별 해석, 모델 라우팅(Codex 우선 Opponent + bridge), 입력/출력 스키마, 라운드 정책, §6.9 admissibility 연결 등 "how"는 [`contracts/dialectic-deliberation-contract.md`](contracts/dialectic-deliberation-contract.md)가 소유한다. 충돌 시 권위는 실제 스키마 > 본 절(what) > 상세문서(how) 순이다(§3.3 일반화).
...
```text
Producer    -> 초안 또는 기존 산출물의 strongest defensible interpretation을 만든다.
Opponent    -> 약한 가정·빠진 대안·scope creep·검증 부족을 공격한다 (Codex 우선, 상세문서).
Synthesizer -> 양쪽 근거를 evidence 기준으로 채택/기각해 합의안/수정안/보류를 만든다.
```

사용 시점(요약): 중요한 설계문서·제안서·PRD·ADR 작성/개정, 기존 산출물 리뷰(승인/수정/기각), 기술·제품 결정 대안 검토, recommendation 생성, 큰 autopilot의 plan/checkpoint 검증, final answer 전 high-impact 검증.
...
핵심 불변규칙(what — 메커니즘·스키마·모델 라우팅은 상세문서):

- Producer/Opponent/Synthesizer는 서로 다른 context/agent로 실행한다. 한 agent가 겸임하지 않는다.
- Opponent는 예의상 반대하지 않는다. 반론은 파일/라인·문서 근거·사용자 의도·acceptance criteria 중 하나와 연결돼야 한다(§6.9 admissibility).
- Synthesizer는 중간값이 아니다. 근거 약한 반론은 기각, 강한 반론은 반드시 반영. critical objection이 남으면 `accept` 불가.
- 한 번의 소규모 토론이 기본. 무한 debate 금지.
- canonical skill은 `/ditto:dialectic` 하나. `--mode`로 분기하고 `/ditto:dialectic-review` 등은 얇은 alias.
- 멀티모델 disagreement는 곧 진실이 아니다 — 추가 evidence/tradeoff 기록의 trigger.
- 이 계약은 작성 품질 검증용이다. 실제 코드 동작 검증은 EvidenceContract·Verifier·E2EJourneyContract를 추가로 통과해야 한다.
...
### 6.9 Convergence Contract

> **상세 설계.** 이 절은 "what"(수렴 정의·두 게이트·세 원칙)만 둔다. admissibility 판정 메커니즘, ratchet(최선본 보존·decision ledger) 구조, 정직 라벨 스키마화, `convergence.json` 사이드카 등 "how"는 [`contracts/convergence-contract.md`](contracts/convergence-contract.md)가 소유한다. 충돌 시 권위는 실제 스키마 > 본 절(what) > 상세문서(how) 순이다(§3.3 일반화).

목적: 반복 정련(review→edit, verify→fix, dialectic 라운드)이 **treadmill(같은 지점 진동·점진적 품질 저하)에 빠지지도, 조기 수렴(명백한 이슈 무시·no-op·정해진 횟수만 돌고 종료)에 빠지지도** 않게 한다. §6.8 Completion Contract의 쌍대(dual)이며 제1원칙의 직접 파생이다.

핵심 구조: 수렴은 게이트 하나로 정의할 수 없다. **방향이 반대인 두 게이트가 동시에 만족되는 교집합**이 고정점이다.

| 게이트 | 막는 고장 | 통과 조건 |
|---|---|---|
| `CompletionGate` (§6.8) | 조기 멈춤, no-op, 거짓 "done" | 모든 acceptance criterion이 증거로 *적극* 충족돼야 STOP/done 가능 |
| `ConvergenceGate` (본 절) | treadmill, 트집 무한 루프, 품질 저하 | grounded·novel·admissible 반론이 있어야 CONTINUE/edit 가능 |

핵심 불변규칙(what — 판정 메커니즘·스키마는 상세문서):

- **수렴 정의**: `done`은 카운트나 "느낌"이 아니라 두 게이트 동시 만족(*모든 AC 증거로 닫힘 + admissible 열린 반론 0개*)으로만 정의된다. 라운드 1에 참이면 라운드 1에 멈춘다. `cap-reached ≠ converged` — 캡 도달 시 미충족이면 non-pass(verdict는 §6.8) + handoff.
- **admissibility**(반론이 행동을 트리거할 자격): (a) acceptance criterion 매핑(oracle) ∧ (b) ledger에 없던 신규(memory) ∧ (c) critical/major severity. 대칭 원칙(기각도 raise만큼 근거). **묵살 불가**: criterion에 매핑된 반론은 inadmissible 선언 불가 — criteria가 바닥이다.
- **ratchet**(단조 비퇴행): 최선본 보존(모든 버전 불변기준 점수 → "최고점 버전"이 산출물, degradation 정의상 불가) + 결정 원장(채택/기각 이유 기록, 새 근거 없는 재제기 자동 각하 → A→B→A 진동 불가).
- **정직 라벨**(제1원칙 투명성): 정체(`finding|hypothesis|taste`)·확신·상태(`acted|deferred|dismissed`+이유). 백킹 없는 주장은 `finding`이 아니라 `hypothesis`로 라벨링하고 행동하지 않는다. 투명성은 계층적(actionable 전면, 나머지 접거나 on-demand — "같은 볼륨으로 쏟기"는 노이즈).

저장 위치:

```text
.ditto/work-items/<id>/convergence.json
```

`convergence.json`은 버전별 점수·decision ledger·deferred/dismissed 항목과 이유를 담는 additive 쌍대 사이드카다. `completion.json`(§6.8)을 대체하지 않는다.
...
skills/
  ...
  handoff/SKILL.md
  autopilot/SKILL.md        # autopilot 드라이버 (main agent 구동, 비노출)
agents/
  researcher.md
  planner.md
  implementer.md
...
- hook은 얇은 glue로 둔다. 의도 분류·모호성·완료 판정 같은 맥락 판단을 hook의 키워드·임계치 휴리스틱으로 굳히지 않고 skill/agent(판단)와 schema(바 정의)로 보낸다(§11 원칙 10). 단 완료/수렴 게이트의 강제력은 schema 제약으로 결정론으로 박는다.
...
| `/ditto:plan` | v0 (비노출) | acceptance criteria 기반 실행 계획 | `plan.md` |
...
- autopilot은 전용 subagent가 아니라 **main agent가 `autopilot` skill로 구동**한다(§7.3, autopilot §3.4) — subagent는 subagent를 spawn 못 하므로 spawn 루프는 main에 산다. main이 stage subagent를 1-레벨로 spawn하고 Stop hook이 루프를 유지하며, autopilot skill은 content 생성 없이 노드 선택·실패 분류·정지 판단만 한다("orchestrator, not implementer"). 그래프 *상태*는 `autopilot.json`(schema), *판단*은 skill 프롬프트(§11 원칙 10), 루프 *지속*은 Stop hook — 세 축을 섞지 않는다. 주류(Sisyphus=primary, OMC Ralph+persistent Stop) 일치.
- 결정 로직을 별도 결정자 subagent로 떼는 분리(HANNES harness 패턴)는 v0 후 결정 context 오염 증거 시의 future refinement다(autopilot §3.5) — v0엔 과잉.
...
- `autopilot.*`: 큰 범위 작업의 autopilot graph와 내부 checkpoint
- `evidence-index.json`: 커밋 가능한 evidence ledger(요약/hash/exit code/freshness/portability). 다른 clone/세션에서 raw 없이 판단하게 한다.
- `convergence.json`: 반복 정련의 버전 점수, decision ledger, deferred/dismissed 항목과 이유. `completion.json`을 대체하지 않는 쌍대 sidecar.
...
- 작은 요청은 바로 처리한다.
- 큰 요청은 기본적으로 autopilot으로 끝까지 처리한다.
...
이 절은 post-v0(M5) E2E 설계다(§12 Scope Authority). E2E executor는 MCP가 아니다. 브라우저 검증은 직접 Playwright/Chromium command와 artifact capture로 수행한다.
...
10. 결정론(코드)과 판단(LLM)의 층위를 섞지 않는다. 코드는 바(bar)를 강제하고, LLM은 무엇이 바를 넘는지 판단한다. 맥락 의존적 판단(의도 분류, 모호성, evidence 충족, admissibility)을 키워드·임계치·정규식 같은 결정론 휴리스틱으로 굳히지 않는다 — 그 동결이 전신 하네스 HANNES의 self-referential mutation cycle 비대화를 불렀다(`reports/harnesses/hannes.md` §5). hook은 얇은 glue(라우팅·IO·lock·schema 검증)로 두고, 판단은 skill/agent 프롬프트로, 바 정의는 선언적 schema로 둔다. 단 게이트의 *강제력*은 결정론이어야 한다 — 완료/수렴 게이트를 프롬프트로만 두면 모델이 자기인증으로 우회한다. 일관성은 코드가 아니라 acceptance criterion이라는 앵커에서 온다.
11. 하네스 구현 언어는 TS/JS로 통일한다(Zod schema 단일 소스). HANNES의 Python hook/script + 별도 JSON schema 조합은 둘 사이 drift와 유지보수 비용을 낳았다. DITTO는 hook을 `.mjs`(§7.1)로, 계약을 `src/schemas/*.ts` Zod(§3.3)로 둬서 schema가 곧 단일 타입 소스가 되어 선언적 스키마 층의 drift를 타입 시스템이 막게 한다. 언어 선택 자체는 부차적이며, 본질은 원칙 10의 판단 배치다. TS/JS 통일은 그 배치를 단일 타입 시스템 안에서 강제하기 위한 수단이다.
...
| `AutopilotStore` | autopilot graph와 checkpoint state |
...
| `CompletionGate` | 완료 판정(조기 멈춤·no-op 방지) |
| `ConvergenceGate` | 반복 정련 admissibility 판정, 최선본 보존, decision ledger(treadmill·조기수렴 방지) |
...
이 절의 milestone은 DITTO 자체를 개발하는 순서다. 사용자의 실제 작업을 한 단계씩 끊어 처리하겠다는 뜻이 아니다. DITTO가 실행하는 사용자 작업은 가능한 한 `AutopilotContract`를 따른다.
...
"v0인가"는 두 축으로 나뉜다: **(a) v0 contract/skeleton** = 문서/스키마/fixture/빈 skill·hook 표면이 존재함, **(b) runtime behavior** = 실제 동작이 검증되는 milestone. 아래 표가 surface별 두 축의 권위이며, **범위 충돌 시 §7.1/§9/§13/§14보다 이 표가 우선한다.** v0 수락은 (a)열을 만족하면 되고, (b)열은 해당 milestone에서 검증된다. drift 중이던 surface만 싣는다(자명한 것은 생략).

| Surface | v0 contract/skeleton (a) | runtime behavior (b) |
|---|---|---|
| `UserPromptSubmit` / `Stop` hook | 파일 + 최소 동작 | M1 |
| `PostToolUse` hook | 파일 placeholder | M3 |
| `PreCompact` hook | 파일 placeholder | M4 |
| verifier 분리 / evidence 기반 완료 | agent `.md` + completion schema + fixture | M3 |
| handoff | handoff skill skeleton + schema | M4 |
| Convergence gate | contract + schema + fixture | M3+ |
| `/ditto:plan` | skill skeleton (autopilot 내부 호출, 비노출) | M2 |
| E2E (`/ditto:e2e`, `playwright-e2e`) | 없음(placeholder도 post-v0) | M5 |
...
- intent/question/autopilot/dialectic/handoff/convergence sidecar schema
...
### Milestone 2: Autopilot skeleton
...
- `autopilot` skill skeleton (main agent 구동: ready 노드 선택 · stage subagent spawn · 실패 분류, §6.5)
- fixable failure 재시도 정책 (실패 분류 → retry/switch/escalate)

완료 기준:

- approval gate를 통과한 단일 요청에서 plan -> implement -> verify checkpoint가 사용자 개입 없이 이어진다.
- main agent가 `autopilot` skill로 ready 노드를 골라 stage subagent를 spawn하고, 노드 실패를 분류해 retry/switch/escalate를 결정한다.
...
- completion JSON 생성
- Convergence gate runtime: admissibility 판정, 최선본 보존, decision ledger(`convergence.json`)

완료 기준:

- 테스트/빌드/브라우저 명령 결과가 evidence로 남는다.
- verifier가 acceptance criteria별 pass/fail/unverified를 기록한다.
- 반복 정련이 두 게이트(완료↔수렴)로 종료되고, 캡 도달은 non-pass로 닫힌다.
...
| 위험 | 대응 |
|---|---|
| hook 복잡도가 커짐 | v0 hook 표면을 UserPromptSubmit/Stop/PreCompact/PostToolUse 4개 파일로 제한한다. 동작은 M1에서 UserPromptSubmit/Stop만, PreCompact/PostToolUse는 M3~4에서 채운다(§12 Scope Authority 우선). |
| 자동 질문 억제가 필요한 질문까지 막음 | `QuestionGate` decision과 risk를 evidence로 남기고 deep interview escape를 둔다. |
| autopilot이 너무 큰 작업을 무리하게 계속함 | plan approval gate, checkpoint별 budget, blocker 분류, handoff continuation을 두되 root goal은 임의 축소하지 않는다. |
...
6. 큰 범위 작업은 하나의 autopilot으로 계획-구현-검증-수정-완료까지 이어진다.
7. 내부 checkpoint 완료만으로 사용자-facing 완료를 선언하지 않는다.
8. 완료 판정은 acceptance criteria별 evidence와 연결된다.
9. context pressure 또는 중단 시 handoff가 생성되고 같은 autopilot으로 이어진다.
...
14. 반복 정련의 종료는 카운트가 아니라 두 게이트(완료↔수렴) 동시 만족으로 정의되고, criterion-linked 반론은 묵살할 수 없으며, 백킹 없는 주장은 finding이 아니라 hypothesis로 라벨링된다.

1~9, 12, 13, 14는 v0(Milestone 0~2) 수락 기준이다. 10~11은 post-v0(Milestone 5~6) 수락 기준으로, v0 완료 판정에는 포함하지 않는다. 단 기준 4·8·9·14의 v0 수락은 **계약/skeleton 수준**이며, 실제 runtime 동작이 검증되는 milestone은 §12 Scope Authority를 따른다(verifier/evidence·convergence=M3, handoff=M4).
...
2. `AutopilotContract` contract와 fixture
...
plugin skeleton은 §11 원칙 10(결정론↔판단 층위)을 처음부터 박아서 HANNES의 (B)형 누출(판단을 hook 키워드·임계치로 동결, `reports/harnesses/hannes.md` §5.1)을 답습하지 않는다. surface별로 무엇이 **코드(glue)**, **schema(바)**, **프롬프트(판단)** 인지 갈라 둔다.

| Surface | 코드(`.mjs` glue) | schema(바 정의, Zod) | 프롬프트(판단, skill/agent) |
|---|---|---|---|
| `UserPromptSubmit` hook | work item load/create, hook input 파싱, `runtime-log.jsonl` append | `work-item.schema`로 연결 검증 | 의도 분류·질문 필요성·deep-interview 필요성 — **키워드 표 금지**, advisory로 시작(§7.2) |
| `Stop` hook | `completion.json`/`convergence.json` 읽기, handoff 존재 확인 | `final_verdict=pass ⇒ 모든 AC pass ∧ in-scope unverified 0`을 schema 제약으로 **결정론 강제**(자기인증 우회 차단) | (M3+) convergence admissibility 판정은 LLM, 단 결과를 schema 제약에 묶어 기록 |
| `/ditto:*` skill | 없음(skill은 순수 판단 층) | 산출물(`interview.md`/`dialectic-<n>.json`/`completion.json` 등)만 schema 검증 | deep-interview·dialectic 3역·verify·handoff 전부 프롬프트 |

원칙: hook은 "사실 수집 + schema 강제"만 하고 의미 판단을 품지 않는다. 게이트의 *강제력*은 schema 제약(코드)으로, *무엇이 게이트를 넘는지*는 프롬프트로 분리한다. M1에서 `UserPromptSubmit`/`Stop`의 판단 부분은 advisory(차단 아님)로 시작하고, 강제 block은 schema 위반(완료/수렴)에만 건다(§13).
