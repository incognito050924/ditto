...
- **엣지**: `depends_on`으로 표현하는 DAG. **cycle 금지는 *순서 엣지(`depends_on`)* 한정**이다 — `selectReadyNodes`(ready = 모든 depends_on이 passed)의 교착을 막고 forward-progress를 보장하는 liveness 장치다. 반복(수정→재리뷰)은 같은 노드로 돌아가는 back-edge가 아니라 §2.4 **forward 재확장**으로 표현하므로 이 불변식과 충돌하지 않는다.
...
**`e2e` 노드 트리거(언제 포함하나).** `e2e`는 항상 전개되는 표준 단계가 아니다 — planner는 **high-impact web/user-journey 변경**, 즉 페이지·라우트·UI 컴포넌트·사용자 흐름(로그인·결제·폼 제출 등)에 영향을 주는 acceptance criterion이 있을 때에만 `e2e` 노드를 그래프에 포함한다(`ditto:e2e` skill 트리거 기준과 동일: "high-impact web changes that affect a user journey"). 비-web 변경(라이브러리·CLI·내부 로직)에는 e2e를 강제하지 않는다. e2e 노드가 그래프에 들어오면 이후 경로는 이미 배선돼 있다 — playwright-e2e가 journey 산출물(`.ditto/runs/<id>/`)을 `evidence_refs`로 보고하고(`autopilot-loop.ts` recordResult), `deriveAcVerdicts`(`autopilot-complete.ts`)가 그 evidence로 해당 acceptance를 닫는다. 끊겨 있던 것은 이 트리거 규칙뿐이었다.
...
- **초기 그래프는 고정 템플릿이 아니다.** 부트스트랩은 최소 씨앗만 두고, 실제 노드 집합은 `design`(planner) 노드가 작업 규모에 맞춰 *생성*한다(§2.4). 모든 전개는 노드 단위이며 root_goal·work item은 단일하게 유지된다(분리 없음, §7).

### 2.4 그래프 생성과 확장 (planner = 그래프 생성기 · forward 재확장)

그래프는 부트스트랩 시점에 박제된 고정 노드 집합이 아니다. 부트스트랩은 최소 씨앗만 두고, **`design`(planner) 노드가 root_goal·acceptance_criteria를 보고 작업 규모에 맞는 노드 서브그래프를 생성**한다 — 작은 작업은 소수 노드, 큰 작업은 표준 라이프사이클(§2.2) 전개. 즉 planner는 "계획 산문만 내는 read-only 노드"가 아니라 **그래프 생성기**다. root_goal은 불변, 전개는 노드 단위(§2.3).

- **접합**: planner가 산출한 노드(들)는 `AutopilotStore`의 노드-추가 연산으로 그래프에 접합된다 — 비순환 검증·id 안정성(기존 노드 id 불변)·schema 검증을 통과해야 한다. 드라이버가 파일을 직접 쓰지 않는다(§2.3).
- **forward 재확장 (노드 *간* 수렴 루프)**: "이슈 0까지 수정+재리뷰"는 같은 노드로 돌아가는 back-edge가 아니라 *앞으로 덧붙이는* 노드로 표현한다. findings를 낸 `review`(또는 `security`) 노드는 새 `fix` 노드 + 새 `review` 노드를(새 id, 엣지 단방향, `depends_on`=직전 노드) 생성한다. 엣지가 항상 앞을 향하므로 DAG·cycle-금지 불변식(§2.1)이 유지되고 §3.3 file-overlap gate·liveness 보장이 그대로 적용된다. 루프 종료는 §4.3 **수렴 예산**이 지배한다.
- **두 층의 반복 구분**: forward 재확장은 *노드 간* 반복(새 노드 덧붙임), `attempts` 재무장은 *노드 내* 반복(같은 노드 retry/switch, §4.3·§6.9). 둘은 별개 층이며 섞지 않는다.
...
- **노드 *간* 수렴 루프(forward 재확장, §2.4)의 종료 = 2층 탈출.** (1) **수렴 탈출(닫음)**: `review`/`verify`/`security` 노드가 `findings=0`을 *evidence와 함께 판정*하면 루프를 닫는다(pass). 닫을 권한은 에이전트 verdict에만 있다 — "수정했다"는 *신호*가 아니라 *판정*이 닫는다(G7 content-free 가드와 정합). (2) **예산 탈출(멈춤·닫지 않음)**: 재확장 라운드/토큰/시간 **수렴 예산**이 소진되면 *멈추되 닫지 않는다* — `cap-reached ≠ converged`와 동형으로 escalate(`user_decision_needed`), **절대 pass 아님**. 즉 정량 예산은 *멈출 권한*만 있고 *통과시킬 권한*은 없다(원칙 10: 판단=에이전트, 강제=결정론).
- 수렴 예산의 구체 필드(예: `caps.converge_rounds`, 또는 토큰/시간 예산)는 구현 시 schema에 확정한다(`[VERIFY]` — §0 권위 규칙: 예시 JSON은 실제 Zod/JSON 스키마와 맞춘 뒤 반영).
...
- [ ] 초기 그래프는 고정 템플릿이 아니다 — planner가 작업 규모에 맞춰 노드를 생성·전개한다(§2.4). 반복은 back-edge가 아니라 forward 재확장(엣지 단방향, cycle 금지 유지).
- [ ] 노드 간 수렴 루프: 닫는 권한은 에이전트 verdict(`findings=0`+evidence)에만. 정량 수렴 예산은 멈춤·escalate일 뿐 pass 아님(§4.3).
