...
- [ ] 계약이 §9 런타임 산출물(파일·사이드카) 또는 런타임 동작을 기술하면, 그 항목의 AC는 "런타임 생성·배선" evidence를 최소 1개 요구한다 — 그 파일이 런타임 경로에서 실제로 생성되거나, 그 메커니즘이 런타임 루프에서 호출됨을 검증해야 하며, 직렬화/순수 함수 단위 테스트로 대체하지 않는다(wi_260614ojc 갭 §4-A1: ac-6이 serializePlanDialog STRING만 검증, 파일 write caller AC 부재).
- [ ] "존재" AC(X가 정의되고 단위 테스트 통과)와 "배선" AC(X가 autopilot 런타임 루프에서 호출되어 Y를 산출)를 분리한다 — "X가 정의되고 테스트 통과"류는 배선을 함의하지 않으므로, 배선이 필요한 항목은 별도의 행위 AC로 명문화한다(wi_260614ojc 갭 §4-A2: ac-4·ac-9가 "0 fail"로만 정의돼 배선 부재를 통과시킴).
