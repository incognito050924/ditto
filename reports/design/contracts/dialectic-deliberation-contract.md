...
| §6.5 One-Shot Orchestration | 그래프 진행 드라이버 | Dialectic은 high-impact 노드(`review`·plan·architecture)에서 드라이버가 *호출*하는 검증 단계다(one-shot §2.2 review owner = "dialectic 3역"). 드라이버는 content를 안 만들고, Dialectic의 3역이 만든다. |
...
### 3.4 Codex Opponent bridge (별도 v0 항목 — `run with` 재사용 금지)

외부 provider 호출은 raw CLI를 직접 조립하지 않고 **wrapper/bridge**를 통해 실행해 `.ditto` evidence와 연결한다(메인 §6.6). 핵심 규칙:

- 이 Opponent bridge는 **격하된 `run with`(§3.3)를 재사용하지 않는 별도 v0 설계 항목**이다. `run with`는 smoke/debug/manual capture로 격하됐고(메인 §3.3), Claude Code native session/permission/hook lifecycle을 우회하므로 primary 경로로 쓰지 않는다(메인 §3.3 "run with 처리"). Dialectic의 Opponent 경로는 이를 답습하지 않는다.
- bridge는 메인 §11 core interface `OpponentModelRouter`("Codex 우선 반대자 라우팅과 Claude fallback 기록")가 책임진다. 라우팅 *판단*은 router/agent 프롬프트, 호출 *glue*(CLI 조립·IO·타임아웃·결과 캡처)는 얇은 코드 층, 결과 *기록*은 schema에 둔다(메인 §11 원칙 10: 결정론과 판단의 층위 분리).
- bridge 산출은 `dialectic-<n>.json`의 `opponent.run`에 provider/model/command/timestamp/exit·fallback을 남겨 evidence로 추적된다(§3.5, §5.3).

> v0 범위 주의: Codex bridge는 **있으면 우선** 경로다. doctor가 런타임에 Codex 가용성을 확인하고(메인 §0 "v0: opponent/reviewer lane for dialectic checks when available"), 불가면 §3.2 fallback이 정상 경로다. Codex 불가 자체는 실패가 아니다.
...
mode 집합은 v0 고정이다. 새 mode 추가는 본 문서 개정으로만 한다(drift 방지 — deep-interview §4.1·one-shot §2.2와 동형). alias(`/ditto:dialectic-review` 등)는 `--mode`만 박은 얇은 wrapper이지 새 mode가 아니다.
...
- **cap_reached ≠ converged.** max_rounds 도달 시 두 게이트를 못 채우면 그건 done이 아니라 non-pass다. 최종 verdict(`partial|fail|unverified`)와 work item status는 §6.8 CompletionContract가 결정하고 §6.10 handoff한다(메인 §6.9, one-shot §4.3과 동형).
