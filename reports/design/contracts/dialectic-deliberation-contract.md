...
Dialectic Deliberation은 **결정·검토·제안·문서·답변 같은 high-impact 산출물에서, 한 모델 한 context의 자기 확신을 깨기 위해 Producer(최선 주장)·Opponent(근거 있는 공격)·Synthesizer(근거 기반 합성)를 서로 다른 context로 분리 실행하고(1차 불변식), 가능하면 다른 provider/model로도 분리해(2차 이득), 더 강한 합의안 또는 명시적 보류 판단을 만드는 작성 품질 검증 계약**이다.

핵심 가설(실측 근거 있음): **동일 모델 계열 단일 context는 공통 맹점을 공유한다.** HANNES `mi-scope-fix`는 Stage 4 critic이 전부 Claude opus 동일 계열이라(`hannes.md` §3.2) 작업을 close한 *뒤에야* codex+critic 적대 리뷰에서 BLOCKER 2건이 드러나 `mi-scope-redesign`으로 재작업했다(`hannes.md` §3.2). 그 가장 비싼 lesson을 DITTO는 설계에 내장한다 — 1차 불변식은 **Producer/Opponent/Synthesizer의 context 분리**, 2차 이득은 가능할 때의 provider/model 다양성이다(§3).
...
같은 context가 초안을 쓰고 곧바로 자기 초안을 비판하면, 이미 커밋한 가정을 방어하는 *확증* 자세로 미끄러진다. 분리의 목적은 두 가지다 — (a) **context 분리**: Opponent는 Producer의 사고 과정·합리화를 받지 않고 산출물만 본다. 이는 필수다. (b) **모델/provider 분리**: 가능하면 Opponent를 다른 provider/model로 두어 동일 계열 맹점을 더 강하게 깬다(§1.1). 모델 다양성이 불가능해도 같은 main context에서 3역을 흉내 내면 안 된다.
...
| Opponent on Claude Code host | **codex via codex-plugin-cc** | codex via codex-plugin-cc | claude-opus → claude-sonnet (Codex plugin 불가 시, 별도 context 유지) |
| Opponent on Codex host | **codex separate context** | codex separate context | custom agent 불가 시 generic Codex subagent; Claude Code 역호출 없음 |
| Synthesizer | claude-opus 또는 claude-sonnet | claude-opus | — |

목적: 먼저 3역을 서로 다른 context/agent로 나눠 단일 context 자기확신을 끊고, 가능하면 Opponent의 provider를 바꿔 "같은 모델 계열 스타일의 공통 맹점"까지 줄인다(메인 §6.6). Claude Code host에서는 codex-plugin-cc로 provider 다양성을 얻는다. Codex host에서는 이미 Codex가 current host이므로 Claude Code를 역호출하지 않고, 별도 Codex subagent/context를 강제한다.

### 3.2 Context 분리와 provider fallback

context 분리는 fallback 대상이 아니다. Producer, Opponent, Synthesizer가 같은 main context 안에서 역할극으로 처리되면 dialectic은 실패다.

- **Claude Code host**: Codex plugin for Claude Code(codex-plugin-cc), 인증, 네트워크, 비용, runtime 상태 때문에 Codex를 못 쓰면 Opponent는 **claude-opus → claude-sonnet** 순으로 fallback한다. fallback은 침묵하지 않는다 — artifact에 fallback 발생과 사유를 남긴다(§3.5). 동일 계열 Claude라도 분리된 context로 돌면 단일 context보다는 낫지만, provider 다양성 이득은 줄어든다는 점을 verdict 신뢰도에 반영한다.
- **Codex host**: Claude Code로 fallback하지 않는다. `dialectic-opponent` custom agent/subagent가 callable이면 그것을 쓰고, 아니면 **generic Codex subagent**에 Opponent packet과 지침을 넣어 별도 context를 만든다. custom role 불가는 provider fallback이 아니라 role-surface downgrade이며, `opponent.run.command`에 기록한다. Codex host에서 별도 subagent/context spawn 자체가 불가능하면 deliberation은 `blocked`다.
...
### 3.4 Codex Opponent 호출 = host-aware delegation

Opponent의 실행은 current host에 따라 다르다. 라우팅 *판단*은 ditto의 `OpponentModelRouter`(`src/core/opponent-router.ts`, 메인 §11 core interface)가 결정론으로 맡고, 실제 *호출*은 main agent가 현재 host의 안전한 표면으로 수행한다. 결과 *기록*은 schema(`opponent.run`)에 남긴다. (메인 §11 원칙 10: 판단/실행/기록 층위 분리.)

- **Claude Code host**: Codex는 Claude Code용 Codex 플러그인(codex-plugin-cc)이 세션에 있을 때 available이다. task형 Opponent는 `codex:rescue` 위임, review형은 플러그인의 adversarial-review를 사용한다. 이 경로는 Claude Code host 전용이다. Codex 후보가 unavailable이면 `opponent_fallback`의 Claude 후보를 별도 `dialectic-opponent` context로 실행한다.
- **Codex host**: `resolveOpponentCandidates(model_policy, { currentHost: 'codex' })`는 Codex provider 후보만 남긴다. Claude Code 역호출 fallback은 없다. `dialectic-opponent` custom agent/subagent가 callable이면 그것을 직접 spawn한다. callable이 아니면 **generic Codex subagent**를 별도 context로 spawn하고, Opponent 역할 지침과 Producer 산출물만 전달한다. generic subagent도 불가능하면 blocked다.
- **공통**: host 표면을 우회해 내부 스크립트(예: `codex-companion.mjs`)를 직접 부르지 않는다(버전 경로·lifecycle 우회 회피). 산출은 `dialectic-<n>.json`의 `opponent.run`에 provider/model/command/timestamp·fallback을 남겨 evidence로 추적된다(§3.5, §5.3). Codex host의 custom-agent → generic Codex subagent 전환은 provider fallback이 아니므로 `fallback_from=null`, `fallback_reason=none`을 유지하고 `command`에 downgrade를 남긴다 — `selectOpponent`가 선택 후보의 provider가 preferred와 같으면 `fallback_from`을 null로 두어 이를 코드 레벨에서 보장한다(`opponent-router.ts`).

> v0 범위 주의: Codex 불가 자체는 Claude Code host에서는 실패가 아니다. Codex host에서는 Codex가 current host이므로 custom role 호출 실패가 곧 provider 실패가 아니다. 별도 context의 generic Codex subagent로 계속할 수 있으면 dialectic은 진행한다.

### 3.5 provider run 기록 (정직성)

Opponent 실행 artifact에는 provider, model, command, timestamp, 실패/fallback 여부를 남긴다(메인 §6.6). 기존 `providerName` enum(`codex|claude-code|opencode|openagent|other`, `common.ts:33-35`)과 `runId`(`run_…`, `common.ts:16-19`)를 재사용한다 — 새 provider enum을 만들지 않는다. `command`는 호출 표면을 구분할 만큼 구체적이어야 한다: 예를 들어 Claude Code host의 `codex-plugin-cc:codex:rescue`, Codex host의 `custom-agent:dialectic-opponent`, `generic Codex subagent:dialectic-opponent-packet`.
...
`model_policy.opponent_fallback`의 Claude 후보는 Claude Code host에서만 provider fallback으로 해석된다. Codex host에서는 `OpponentModelRouter`가 Codex provider 후보만 남기며, custom agent 불가 시의 generic Codex subagent 사용은 `opponent.run.command`에 기록되는 role-surface downgrade다. TOKEN_MAP에 없는 unknown 토큰은 명시된 `currentHost`의 provider로 해석된다(라우터는 모델명 유효성을 검증하지 않음 — 그 책임은 driver). 따라서 같은 provider 안의 후보 전환은 `selectOpponent`가 `fallback_from=null`/`fallback_reason=none`으로 기록하고, provider가 실제로 바뀔 때만 `fallback_from`에 preferred provider를 남긴다.
...
- [ ] Opponent는 별도 context가 필수다. Claude Code host는 Codex 불가 시 claude-opus → claude-sonnet fallback을 기록하고, Codex host는 Claude Code 역호출 없이 custom agent 또는 generic Codex subagent를 쓴다.
- [ ] Codex Opponent는 host 표면으로 호출한다 — Claude Code host는 codex-plugin-cc(`codex:rescue`/adversarial-review), Codex host는 `dialectic-opponent` custom agent 또는 generic Codex subagent 직접 spawn. 내부 스크립트 직접 호출·격하된 `run with`(§3.3) 재사용 금지.
- [ ] 같은 provider 내 surface 전환(Codex custom-agent → generic subagent)은 provider fallback이 아니다 — `fallback_from=null`/`fallback_reason=none`을 유지하고 surface는 `command`에 기록한다(§3.4, `selectOpponent`가 코드로 보장).
