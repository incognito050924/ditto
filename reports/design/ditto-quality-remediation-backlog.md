...
1. **`completion-store.ts:124-136` mirror-write** — completion verdict + **raw evidence**를 Record AC에 복사(digest-only 규칙 정면 위반, work-item 코드에서 안 보임). 역방향 `:95-110`이 raw를 다시 읽어 **Record가 digest만 가지면 BREAK**. 최고 위험.
...
이들 중 1·2·6은 WS0-T0 상세설계에서 라우팅 확정, 4·5는 값 결정(어느 tier)이라 사용자·도메인 판단 필요. Stop 면제(3계급)는 WS2-T1(경량 경로 보존)과 함께.
...
*R 운영 준비도 = 기존 risk taxonomy 재사용 (net-new 아님).* R은 새 체크리스트가 아니라 **기존 23 카테고리(`coverage-taxonomy.ts:38-131`)의 운영 subset을 readiness로 뒤집은 것**. 같은 렌즈, 프레이밍만 반대("실패하나?"→"준비됐나?"). 무발명·재사용.
...
**함의**: R이 재사용으로 판명 → moat의 진짜 net-new는 **E + 가치/방향 lens(A2·A3·M)**뿐. 빌드 비용 또 축소.
...
- **WS0-T0 · 기록/실행 분리 + 공유 tier 이동 (아키텍처 코어)** *(설계·pre-mortem 먼저)*
  ...
  [검증] Record가 커밋되어 `git ls-files`에 나타남; Run을 삭제해도 Record·verdict 무손실; 경량 경로 Record가 Run 없이 done 도달. **소비자 스윕(§2.5) 반영**: (i) 상태 writer 6곳이 한 event로 수렴, `work-item-handoff.ts:324-341` 직접 write 제거; (ii) `stop.ts:902` strong-block에 "경량 Record(Run 없음) 면제" 추가 — 경량 Record가 매 Stop에 안 걸림(재현); (iii) `completion-store.ts:124-136` mirror가 raw evidence 아닌 digest만 Record에 씀.
  [무게] 무거움 / 다표면·비가역·마이그레이션 동반. [의존] 없음(모든 것의 토대). **dialectic-review + pre-mortem 선행 필수.**
...
- **WS0-T3 · GitHub 칸반 = 두 번째 projection (SoT 아님, 불변식 2)**
  ...
  [검증] Record 상태 변화가 보드 카드로 투영(P₀ dogfood + non-ditto 타깃의 보드에서도 — 명제 I); SoT 역전 없음(보드→ditto 강제 sync 없음).
...
- **WS0-T4 · 기존 63개 로컬 WI 마이그레이션**
  ...
  [검증] 이관 후 전 항목이 `work list`에 나타나고 verdict 보존; 롤백 절차 실증. **leak-test 필수(§2.5 소비자 스윕)**: 디렉터리 통째 commit 금지 — Record 절반만 분해 commit, commit tier에 Run 아티팩트(evidence/·runs/·*-state.json·session 버퍼) 0건 재현. Run 없이도 디렉터리를 자동 생성하는 ~20 생산자가 private 데이터를 committed tier로 새지 않음을 확인.
...
- **WS-HND-T0 · 핸드오프를 recipient-scoped 일회성으로 재정의 (아키텍처)** *(설계·pre-mortem 먼저)*
  ...
  [§2.5 정합 — P0 후 추가] 두 seam을 §2.5 Record/Run 분리와 맞춰야 한다:
    ...
    · **(b) inline durable 필드의 은퇴 누수 (P0 미고려 gap).** `critical_decisions`·`irreversible_risks`(`handoff.ts:31-51`)는 "재도출 불가라 inline 보존"하는 durable 내용인데, 핸드오프는 소비 시 은퇴한다 → §2.5 규율("durable→영속 공유 tier")과 충돌: 은퇴 그릇에 영속 결정이 담겨 사라진다. **소비 시 이 결정을 Record(또는 ADR/knowledge)로 흡수 후 핸드오프만 은퇴** — WS-HND-T4 "결정은 남기고 산문은 은퇴"를 핸드오프 자신의 inline 필드에도 적용.
...
- **WS-HND-T4 · 신선도 민감 설계/기획 산출물 은퇴 (§2.6 원칙 집행)**
  ...
  [파일] tech-spec store, `.ditto/specs/`, knowledge-curator 흡수 경로, WS-HND GC
  [검증] work item done 시 연결된 spec의 durable 결정이 ADR/knowledge로 흡수되고 원 산문이 은퇴됨(재현).
  [무게] 중. [의존] WS-HND-T1(GC primitive), §2.6.
...
- **WS-PRISM-T0 · tech-spec 진화·대체 (아키텍처)** *(설계·dialectic-review 먼저)*
  ...
  [파일] `skills/tech-spec/SKILL.md`(진화), `src/core/tech-spec.ts`, `tech-spec-state.ts`, 기존 `.ditto/specs/*.md` 마이그레이션
  [검증] vibe 입력이 발산·가치 압박을 거쳐 구체화된 설계로 산출되고 deep-interview가 AC로 수렴(종단 재현). **비회귀 = 구체 compat 표면 목록으로 실증(P0 dialectic)**: CLI 명(`cli/index.ts:68` `tech-spec`)·섹션 ID·compile input·기존 테스트(`tech-spec-template.test.ts:54-62`)·기존 `.ditto/specs/*.md` 7개 문서. replace-in-place는 이들에 alias/마이그레이션 계획을 동반해야 하며, "비회귀"를 이 목록 각 항목의 pass로 정의.
...
- **WS-PRISM-T1 · 발산 (옵션 공간 확장) — 신규 (net-new 메커니즘, 설계 §2.7)**
  ...
  [검증] 한 vibe에서 사용자가 못 본 방향 포함 물질적으로 다른 3~5개가 제시됨.
...
**P0가 남긴 열린 결정 → 전부 닫힘(2026-07-06):** Q11(비-저자 감사) → 재현레시피+해시(§7)·§2.7 발산 기계적 상한 → relevance-gate+admissible-only+cap≤5 설계(§2.7)·Q10(non-ditto 타깃) → boxwood 고정(스택 미확인 flag, §7). WS0-T0/WS-PRISM-T0 상세설계 착수 가능 상태. 단 §4 P0의 pre-mortem 절반은 미실행(dialectic-review만) — 원하면 별도.
