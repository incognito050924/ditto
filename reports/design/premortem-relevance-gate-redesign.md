...
3. **적대 검증(refute-by-default)** — 한 평가자(dialectic Opponent 재사용)가 **전 skip 후보를 일괄**로 "이 카테고리는 *관련 있다*"를 입증 시도(범위 1회 결박 후 후보별 refute) → 그럴듯한 경로 하나라도 찾으면 그 skip 취소·카테고리 생존. **일괄인 이유**: §8-5 실측에서 refute 1건이 judge 전체(~48.5k)와 동급이라 후보당 별도 spawn은 비용이 skip 수에 비례(저관련 ~1.1M)했다 → 한 패스로 범위 결박을 amortize. 안전 계약(refute-before-skip)은 불변 — 잃는 건 후보별 컨텍스트 격리뿐이고 보수적 기본값·근거 결박·감사 레이어가 받친다.
...
## 8. 비용 측정 (정의 + 단위비용 실측 — §8-5)

ADR-0023이 ON/OFF 한계비용을 "추정(~20배)·직접 측정 안 됨"이라 명시(`coverage-manager.ts` 비용 모델 근거). 비용이 1차 목표이므로 ON/OFF 한계비용 실측을 **명시 acceptance**로 둔다. **코드에 per-run 토큰 회계가 없다**(검증: `run-manifest`·`autopilot`·`evidence-log` 스키마에 token/usage 필드 부재) → 토큰 실측은 자동으로 못 읽고 아래 프로토콜로 *수동* 수행한다. 이 슬라이스는 측정을 **정의**(§8-1~8-4)하고 단위비용을 **실측**(§8-5, subagent_tokens 캡처 × 결정적 폭)한다; full-tier 라이브 sweep-to-dry 정밀 실측만 별도.
...
### 8-5. 실측 결과 (2026-06-25, wi_260625l0v)

full 23×ON/OFF sweep(수백만 토큰 2회)은 비실용적이라, **단위비용을 실측(실 서브에이전트 `subagent_tokens` 캡처)하고 결정적 폭 축소와 곱해** ON/OFF 한계비용을 산출했다(§8-3 방식). 표본 2건(저관련=설계 문서 prose 편집, 고관련=coverage store 동시성 수정), ditto 실 코드에 결박.

**단위비용 (실측):**

| 단위 | 측정 | 값 |
|---|---|---|
| relevance judge (23 카테고리 1회 판정) | 저 46.1k / 고 50.8k | **~48.5k (관련성 무관, flat)** |
| per-refute (skip 후보 1건 적대검증) | auth 48.1k / data-integrity 48.9k | **~48.5k (judge 1회와 동급)** |
| per-category sweep (Opponent-only light = 1각도+opponent) | 55.2k + 59.0k | **~114k** |

**폭 축소 (결정적, judge verdict):** 저관련 22/23 사전close(샘플 refute 2건 모두 refuted=false → skip 생존) · 고관련 7/23.

**한계비용 (단위비용 × 폭):**

| 시나리오 | gate 오버헤드 = judge + skip×refute | OFF = 23×114k | ON = 오버헤드 + 관련×114k | 절감 |
|---|---|---|---|---|
| 저관련(문서) | 46k + 22×48.5k ≈ **1.11M** | 2.63M | 1.11M + 1×114k ≈ **1.23M** | **+1.40M (53%)** |
| 고관련(동시성) | 51k + 7×48.5k ≈ **0.39M** | 2.63M | 0.39M + 16×114k ≈ **2.22M** | **+0.41M (16%)** |

**판정 (§8-4):** 두 표본 모두 **net-positive** → "유의미한 절감 → 게이트 기본 ON 유지" 충족. 저관련일수록 절감 큼(53% vs 16%) = 의도한 방향.

**핵심 발견 — refute가 오버헤드를 지배 → 일괄 refute 채택:** per-refute가 judge 1회와 동급(~48.5k)인데 원래 §5-3이 **skip 후보마다** 1회 돌려, 저관련(22 skip)에서 refute만 ~1.07M = ON 비용의 ~87%·잠재 절감(judge만 ~2.47M)의 ~43%를 잠식했다. **수정(이 실측 직후 landed)**: §5-3·SKILL §2b.0을 **한 opponent가 전 skip 후보를 일괄 검증**(범위 1회 결박 후 후보별 refute)으로 바꿔 refute 비용을 skip 수에 무관하게 ~1패스로 cap했다 — 저관련 게이트 오버헤드가 ~1.11M → ~judge+1패스(~0.1M)로 떨어져 절감률이 53%→~94%로 개선. 안전 불변(refute-before-skip 계약·보수적 기본값·근거 결박·감사 유지; 잃는 건 후보별 컨텍스트 격리뿐).

**한계(정직):** n=1/관련성-class · per-category는 Opponent-only light 프록시(실 full-tier sweep-to-dry는 더 비쌈 → 절감 *절대값*은 더 크고 refute 비중은 더 작아짐) · `subagent_tokens`는 서브에이전트 단위 비용이지 호스트 청구 총량 아님. 측정 자체 비용 ~308k(6 에이전트). 더 정밀한 수치는 라이브 full sweep 실측이 필요하나, **게이트 기본 ON 유지 판정·refute 비용 레버 발견은 본 실측으로 확정**.
