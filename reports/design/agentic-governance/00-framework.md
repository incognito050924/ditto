...
| `FitnessFunction` mode=deterministic (20 §6) | taint 쿼리 "새 source→sink path = 0" | **실증·배선됨** (`src/acg/fitness/codeql-provider.ts` SARIF→identity + Stop 게이트 가동, `tests/acg/fitness-codeql-provider.test.ts`) |
| `FitnessFunction` baseline.delta_only (20 §6) | "절대 alert 아닌 delta(순증)" | **실증·배선됨** (`src/acg/fitness/fitness-runner.ts` `assessDelta`, `tests/acg/fitness-runner.test.ts`) |
...
3. **Architecture Spec의 출처.** 저장소마다 수동 작성인가, 초기 그래프에서 제안인가? boxwood처럼 경계가 코드에 암묵적으로만 있는 경우 어떻게 부트스트랩하는가? **DITTO 바인딩 결정값(ADR-0004, 구현됨)**: `produced_by=user` 권위 기본의 하이브리드 부트스트랩 — agent는 의존/import 그래프에서 관측 가능한 `layers`/`public_surfaces`만 **비권위 candidate**로 제안하고 사람이 비준해 권위화한다. `forbidden_dependencies`·`layers.can_call`는 관측에서 자동 박제 금지(현재 위반을 규칙으로 굳히는 것 방지). 활성 선결: layers 분류 PoC + 기계 판독 invariant 표현(주석 규약은 파싱 불가). **현재 상태(구현됨)**: candidate 제안기 `src/acg/architecture/propose.ts`(비권위, `forbidden_dependencies=[]` 자동박제 금지)와 소비처 단계6 boundary 게이트 `src/acg/boundary/*`가 구현됨(`tests/acg/architecture-propose.test.ts`·`boundary.test.ts`). 사람 비준으로 권위화하는 출처/권위 정책은 그대로. 스키마는 `produced_by=agent|user`를 허용(변경 없음).
4. **적합성 함수의 비용.** 변경마다 전역 적합성을 다 돌리면 비싸다. 무엇을 변경 시점에, 무엇을 주기적으로 돌리는가? incremental 평가가 가능한가? **DITTO 바인딩 결정값(ADR-0004, 구현됨)**: evaluator.mode별 차등 — deterministic 싼 가드=`per_change`(세션 시작 DB 1회 캐시 후 쿼리만 증분 ~3.9s), 전체 스위트(34s~1분)·executed(컴파일 build 수 분)=`risk_tiered/sampled`+`periodic/on_release`로 **매 변경 전수 금지**. **안전 불변식**: risk base는 `risk_default`(수동 enum)이며 ImpactGraph/boundary 입력 부재·`journey_unknown` 시 high-risk로 **escalate(fail-closed), sample down 금지**. incremental은 "쿼리=증분(캐시 DB)·추출=언어당 전역"으로 정직 한정(per-file 추출 미실증). delta_only는 정규화 `violation_identity` recipe가 생기기 전까지 조건부 유효(부재 시 보고 fail-closed). **현재 상태(구현됨 — `executed` 실행만 잔여)**: fitness runner/scheduler(`src/acg/fitness/fitness-runner.ts`의 `scheduleDecision`·`runFitness`)·ICL emit(`src/acg/icl/*`)·ImpactGraph 생산기(`src/acg/impact/*`)가 구현됨(`tests/acg/fitness-runner.test.ts`·`icl-compiler.test.ts`·`impact-graph.test.ts`). 남은 미구현은 `executed` mode의 e2e *실행* provider와 Assurance drift 집계 뷰뿐. 스키마는 `execution.selection`·`baseline.delta_only`·`cadence`를 허용(변경 없음).
5. **Change Map의 표현 매체.** 텍스트(파싱·diff 가능) vs 다이어그램(사람 친화). 둘을 어떻게 단일 출처에서 생성하는가? → [50](50-change-map.md).
6. **completion gate 연결 방식 (스펙 성질 vs 바인딩 결정).** 스펙은 "거버넌스 실패(미해소 high-risk ReviewGraph·미검증 의미변경·적합성 위반)가 완료를 막아야 한다"는 *성질*만 요구한다. *어떻게* 막는가는 바인딩 결정이다. **DITTO 바인딩 결정값(구현됨)**: 별도 gate를 신설하지 않고, `CompletionContract`에 거버넌스 *기록* 슬롯(`acg_governance`)을 더하되 *집행*은 Stop 훅이 ReviewGraph ledger를 직접 읽어 미해소 high-risk 시 continuation을 강제한다(`completion-contract.ts`는 기록 표면, `stop.ts`가 집행 — CompletionContract가 직접 게이팅하지는 않는다). **현재 상태(구현·가동됨)**: Stop 훅이 ReviewGraph ledger(`.ditto/work-items/<wi>/acg-review.json`)를 읽어 미해소 high-risk 시 continuation을 강제하고(`src/hooks/stop.ts`), `CompletionContract`에 옵셔널 `acg_governance` 슬롯이 있다(`src/schemas/completion-contract.ts`). WU-6 완료(`tests/hooks/stop.test.ts`). 방향은 §6의 "CompletionContract 재사용"과 모순되지 않는다 — 재사용하되 거버넌스 입력 슬롯을 더한다. boxwood 바인딩은 자기 완료 메커니즘에 같은 성질을 다르게 배선할 수 있다.
