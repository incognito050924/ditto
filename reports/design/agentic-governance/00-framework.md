...
> **이 문서의 위치.** ACG는 `reports/design/agentic-engineering-change-governance.md`(아이디어 베이스)를 공학적 프레임워크로 승격한 것이다. 아이디어 베이스가 "무엇이 문제인가"를 6개 패턴으로 정리했다면, ACG는 "어떤 검증된 공학 기법 위에, 어떤 스키마와 문법과 표기법으로 그것을 지속 가능하게 만드는가"를 정의한다. 이 문서는 토대와 모델을 담고, 나머지 다섯 문서가 각 축을 명세한다.

> **stack-agnostic.** ACG는 특정 언어·프레임워크·하네스에 의존하지 않는다. 예시는 `boxwood-workspace`(235K LOC Java/Kotlin + 94K LOC TS/Svelte 모노레포)에서 인용하되, 프레임워크 자체는 repo별 knowledge로 주입되는 구조를 가정한다.
...
→ **ACG의 응답**: 변경의 "왜"를 `ChangeContract`(purpose·invariants)와 `decision_ref`(ADR)로 **산출물에 강제 고정**한다. 당위성을 모델의 순간 판단에 맡기지 않고 계약으로 외부화한다. 스타일·품질의 일관성은 `ArchitectureSpec`·리팩토링 기준([40](40-refactoring-criteria.md))·적합성 함수로 코드베이스가 강제한다 — 모델이 매번 일관되길 기대하지 않는다.
...
→ **ACG의 응답**: 이것이 §2.2 **지속적 적합성**의 존재 이유다. 개별 변경의 정당성이 아니라 *누적·증식*을 시계열(Assurance Graph)로 추적하고, 얕은 wrapper·단일 사용 추상화 같은 SLOP 패턴을 리팩토링 게이트([40](40-refactoring-criteria.md))에서 입구부터 차단한다. SLOP은 "한 번 막는" 것이 아니라 "계속 막아야" 하는 것이기 때문이다.
...
| **Information Hiding / Deep Module** | D. Parnas (1972) / J. Ousterhout, *A Philosophy of Software Design* | 리팩토링 허용/금지와 추상화 적용 조건의 근거. 좁은 인터페이스·깊은 구현을 권장하고 얕은 wrapper를 금지한다. | 리팩토링 기준 (40) |
...
## 6. DITTO 기존 자산과의 매핑 (중복 신설 금지)

ACG는 DITTO에 새 계약 6종을 무더기로 추가하지 않는다. 기존 자산을 재사용하고, 비어 있는 자리만 채운다.

| ACG 개념 | 기존 DITTO 자산 | 관계 |
|---|---|---|
...
| 패턴/결정 지식 | `KnowledgeContract`, ADR, `.ditto/knowledge` | **재사용**. 리팩토링 기준·decision_ref의 저장소 |

> 결론: ACG가 **중복 신설하지 않는 것**은 완료·수렴·지식 계약이다 — 기존의 거대 계약(CompletionContract/ConvergenceContract/KnowledgeContract)을 재구성하지 않는다. ACG가 **새로 채우는 것**은 비어 있던 영역의 스키마뿐이다: `ImpactGraph`·`SemanticCompatibility`·`FitnessFunction`(+`AssuranceSnapshot` 시계열)은 순수 신설, `ChangeContract`는 IntentContract의 얇은 투영, `ArchitectureSpec`은 저장소 1회성 선언(knowledge 보관), `ReviewGraph`는 reviewer output 확장이다. [20-contracts.md](20-contracts.md) §0이 각 스키마의 신설/재사용을 명시하고, 이 표와 일치한다.
...
3. **Architecture Spec의 출처.** 저장소마다 수동 작성인가, 초기 그래프에서 제안인가? boxwood처럼 경계가 코드에 암묵적으로만 있는 경우 어떻게 부트스트랩하는가?
4. **적합성 함수의 비용.** 변경마다 전역 적합성을 다 돌리면 비싸다. 무엇을 변경 시점에, 무엇을 주기적으로 돌리는가? incremental 평가가 가능한가?
5. **Change Map의 표현 매체.** 텍스트(파싱·diff 가능) vs 다이어그램(사람 친화). 둘을 어떻게 단일 출처에서 생성하는가? → [50](50-change-map.md).
6. **completion gate 연결 방식.** change governance 실패를 기존 `CompletionContract`에 직접 연결할지, 별도 gate로 둘지.
7. **추상화 깊이의 측정 가능한 정의.** [40](40-refactoring-criteria.md) §3은 "인터페이스 폭 < 내부 복잡도"를 추상화 게이트로 쓰지만, "폭"과 "복잡도"의 구체 측정(공개 메서드 수·파라미터 수·분기 수·LOC 등)은 언어·저장소마다 다르다. caller≥2 규칙도 valid한 단일 사용 deep abstraction을 기각할 수 있다 — 측정 지표와 예외 처리를 저장소별로 보정해야 한다.
...
- [40-refactoring-criteria.md](40-refactoring-criteria.md) — 리팩토링 허용/금지의 형식화 (리팩토링 축).
