...
| `AutopilotContract` — 임의 분할·축소 금지 | 헌장 **§3-0 Plan Integrity** + Autopilot Loop | `AGENTS.md:40-78` | **완성 — 가장 피로 배운 부분** |
...
1. **`lessons.jsonl` 132건 = DITTO 게이트의 기성 fixture/acceptance.** 자가진화 루프(§2.1)는 버리되 그 *산물*(검증된 실패 패턴)은 회수할 가치가 있다. 직결 예:
   ...
   - `L-20260513-plan-integrity-anchor`(Tidy First 오용 + 임의 축소) → `AutopilotContract`(§6.5) fixture.
...
- **§15 "다음 구현 후보" 6개의 fixture 출처를 HANNES lessons로 지정**하면 fixture 작성 비용이 준다(특히 QuestionGate·Autopilot·Completion·Convergence).
