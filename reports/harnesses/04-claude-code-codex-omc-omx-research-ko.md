...
| 프로젝트 | 분석 커밋 | 버전/상태 |
|---|---:|---|
| `Yeachan-Heo/oh-my-codex` | `ff17267baa3a69b7b3578c64edf8cc78a03e2456` | `oh-my-codex` 0.18.7, 2026-06-01 갱신 (이전: `e0465fdc` 0.18.2) |
| `Yeachan-Heo/oh-my-claudecode` | `ed7800dd73b9ef1811b6f777b2017baabda9962a` | `oh-my-claude-sisyphus` 4.14.4, 2026-06-01 갱신 (이전: `1fe17f0e` 4.14.1) |
...
Claude Code에도 goal 기능과 memory가 있지만, 제품 내부 goal semantics를 외부 하네스가 직접 mutate하거나 완전히 대체하는 것은 조심스럽다. OMC는 4.14.2 이후(커밋 `567e39ff`, 2026-05-25) 이 입장에서 한 발 나아갔다. OMC의 ultragoal은 이제 Stop hook에서 `.omc/ultragoal/goals.json` 기반 지속성을 ralph/autopilot과 같은 수준으로 유지하고, PreToolUse hook에서 활성 ultragoal 세션에 Claude `/goal` 스냅샷이 없으면 도구 실행 자체를 차단한다(`ALLOW_ULTRAGOAL_WITHOUT_GOAL=1`로 의도적 바이패스 가능). 즉 OMC는 Claude `/goal`을 단순히 별도 artifact로 우회하는 것이 아니라, 반드시 활성 Claude `/goal`과 연동되어야 하는 hard gate를 PreToolUse에 심었다. 다만 plan/ledger의 authority는 여전히 `.omc/ultragoal/goals.json`과 `ledger.jsonl`이 갖고, Claude `/goal`은 guard condition 역할을 한다는 점에서 제품 내부 goal state를 완전히 대체하는 구조는 아니다.
...
### OMC (oh-my-claudecode): 이전 `1fe17f0e` → 갱신 `ed7800dd` @ 2026-06-01

신규 커밋 48개, 254개 파일 변경. 비교 주제와 관련된 주요 변경은 다음과 같다.

**ultragoal Stop 지속성 + PreToolUse `/goal` hard gate 추가** (커밋 `567e39ff`, `scripts/persistent-mode.mjs:1084–1113`, `scripts/pre-tool-enforcer.mjs:592–616` @ `ed7800dd`): ralph/autopilot과 같은 수준의 Stop hook 지속성이 ultragoal에 추가되었다. 더 중요한 변경은 PreToolUse에서, 활성 ultragoal 세션에 Claude `/goal` 스냅샷이 없으면 도구 실행 자체를 차단하는 guard가 추가된 것이다. 이는 섹션 9.4의 "OMC는 Claude의 `/goal`을 직접 장악하기보다 별도 artifact를 둔다"는 기술이 이제 불완전하다는 뜻이다(해당 섹션 수정 완료). plan/ledger authority는 여전히 `.omc/ultragoal/` artifact에 있고 `/goal`은 guard condition이므로 제품 내부 goal을 완전히 대체하지는 않는다.

**model × agent 호환성 매트릭스 추가** (커밋 `4ad3b7d8`, `docs/agents/model-compatibility.md` @ `ed7800dd`): Prometheus/Hyperplan/Sisyphus/Hephaestus/Oracle/Aletheia/Hermes 역할별로 추천 모델(Claude Opus 4.7, GPT-5.5 high, Sonnet 4.6, DeepSeek V4 Pro/Flash, Kimi K2.5)과 기피 모델을 명시했다. Hephaestus는 GPT-* 계열과 호환되지 않음을 공식 문서로 정리했다. 이것은 OMC가 Claude-only 하네스에서 multi-model 혼합 하네스로 성격이 더 명시화된 것이다.

**Windows hook manifest 및 psmux/cmux 안정화** (커밋 `253a7ad9`, `86be726d`, `10d32f26`, `55fdef96`, `dd828852`, `83453d0b`): native Windows용 hook manifest 재작성, PowerShell worker 기동 경로, cmux team worker spawning 수정. 문서에서 OMC가 Claude Code 제품에 깊게 결합된다고 설명한 내용은 유효하나, 이제 Windows 지원이 명시적 소유 범위가 되었다.

**HUD payload 경고 추가** (커밋 `645c1fff`, `src/hud/payload-estimate.ts` @ `ed7800dd`): 세션 context 압박이 높을 때 사전에 경고하는 HUD 요소 추가. 이는 long-running workflow의 compaction/resume 관리(섹션 10 항목 6)와 직접 관련된다.

**worktree 정리 안전성 강화** (커밋 `745a9234`, `src/lib/worktree-cleanup-safety.ts` @ `ed7800dd`): 섹션 10 항목 7("worktree orphan 방지")이 코드 수준에서 강화되었다.

**`scripts/keyword-detector.mjs` 주요 개선** (`ed7800dd`): 키워드 감지 스크립트에 대규모 변경(155줄 이상 순변경). OMC keyword detector의 신뢰성이 높아졌다.

비교 분석에 사실 변화를 유발하는 항목: 섹션 9.4 수정 완료. 나머지 섹션(6, 8, 10, 부록)의 OMC 분석은 여전히 유효하다.

---

### OMX (oh-my-codex): 이전 `e0465fdc` → 갱신 `ff17267b` @ 2026-06-01

신규 커밋 84개, 212개 파일 변경. 비교 주제와 관련된 주요 변경은 다음과 같다.

**Autopilot FSM 및 supervision boundary 추가** (커밋 `1fe06442`, `9ee1b9fe`, `src/autopilot/fsm.ts` @ `ff17267b`): Autopilot을 상태기계(FSM)로 형식화했다. child phase는 `deep-interview`, `ralplan`, `ultragoal`, `team`, `ralph`, `code-review`, `ultraqa`이며 ralplan supervision boundary와 child phase 감시가 명시적 gate로 구현되었다. 이는 섹션 7.5에서 설명한 workflow chain이 더 엄격한 상태기계로 강화된 것이다.

**팀 조율 프로토콜 추가** (커밋 `f2315940`, `docs/team-coordination-protocol.md`, `src/team/coordination-protocol.ts` @ `ff17267b`): Team Big Five + ATEM-inspired 경량 조율 gate가 추가되었다. independent fan-out 작업은 기존 tmux/lease 프로토콜을 유지하고, 의존성/공유 파일/cross-boundary 작업에서만 closed-loop communication, mutual performance monitoring, backup behavior, adaptability checkpoint gate를 활성화한다. 섹션 7.4에서 "file-backed distributed state machine"으로 설명한 내용이 한 층 더 정교해졌다.

**auth slot hot-swap 추가** (커밋 `7f0a3fa1`, `src/auth/hotswap.ts` @ `ff17267b`): 할당량 소진(quota exhaustion) 시 Codex auth slot을 세션 중단 없이 교체하는 wrapper. 이것은 long-running team/ultragoal 실행의 내결함성을 높이는 새로운 control plane 요소로, 문서의 기존 분석 범위 밖이다.

**Scholastic ontology reviewer agent 추가** (커밋 `32bf2751`, `prompts/scholastic.md`, `src/agents/definitions.ts:361` @ `ff17267b`): Greek-scholastic 전통 기반의 존재론 검토 역할이 네이티브 agent로 추가되었다. `$ralplan` 실행 중 ontology-heavy 검토에서 advisory 역할로 호출 가능하며 consensus gate의 일부는 아니다. OMX의 native agent TOML 설치 구조(섹션 7.1)가 review 역할까지 확장된 사례다.

**ultragoal 재설계: aggregate Codex goal mode** (`docs/ultragoal.md` @ `ff17267b`): ultragoal이 기본적으로 "aggregate goal mode"로 변경되었다. 전체 ultragoal 실행에 단 하나의 Codex goal을 할당하고, OMX가 G001/G002 스토리 상태와 ledger를 소유한다. 동일 스레드에서 G001 완료 후 G002에 새 Codex goal을 만드는 것이 불가능한 제약을 수용한 설계다. 섹션 7.5와 9.4의 OMX ultragoal 설명(artifact-first, hook steering)은 여전히 유효하지만, aggregate mode 기본값과 `get_goal`/`create_goal`/`update_goal` 도구 계약이 명시화된 것을 주목해야 한다.

**`omx explore` 폐기** (커밋 `bfbc2cab`, `src/cli/explore.ts` @ `ff17267b`): `omx explore`가 deprecated 처리되었다. 기존 호출자는 임시 호환성 경로를 유지하나, 신규 저장소 검색은 Codex 표준 도구/subagent 또는 `omx sparkshell`을 권장한다. 이것은 OMX가 외부 하네스 CLI를 줄이고 Codex native 표면에 더 의존하는 방향의 변화다.

**ralplan 플래닝 전용 경계 강화** (커밋 `c46acf9c`, `skills/ralplan/SKILL.md` @ `ff17267b`): ralplan이 실행 중에 write tool 사용을 명시적으로 차단하는 planning boundary를 추가했다. ralplan → ultragoal/team/ralph handoff가 단일 세션에서 실행으로 이어지는 것을 방지한다. 섹션 7.5에서 설명한 workflow chain의 단계 분리가 더 엄격해진 것이다.

**worker skill 추가** (`plugins/oh-my-codex/skills/worker/SKILL.md` @ `ff17267b`): team worker가 ACK, mailbox, 인박스 명령, task lifecycle을 따르는 명시적 skill이 추가되었다. 섹션 7.4 team runtime 분석에서 묘사한 tmux worker 프로토콜이 공식 skill로 문서화된 것이다.

비교 분석에 사실 변화를 유발하는 항목: 기존 섹션 7.4(팀 runtime), 7.5(workflow chain), 9.1(config generator)의 분석은 여전히 방향이 맞으나, aggregate goal mode와 팀 조율 프로토콜은 기능이 증가한 방향이다. 섹션 3.2에서 "일부 lifecycle과 팀 orchestration이 덜 제품화되어 있다"는 설명은 점점 정도가 약해지는 추세임을 유의해야 한다(분석 내 해당 위치에 각주 추가 없이 이 갱신 섹션으로 대체한다).
...
- OMC, `Yeachan-Heo/oh-my-claudecode`, commit `ed7800dd73b9ef1811b6f777b2017baabda9962a` (갱신: 2026-06-01; 이전: `1fe17f0e19ec9e24c5964d4363a9f53aab45c73f`): https://github.com/Yeachan-Heo/oh-my-claudecode/tree/ed7800dd73b9ef1811b6f777b2017baabda9962a
- OMX, `Yeachan-Heo/oh-my-codex`, commit `ff17267baa3a69b7b3578c64edf8cc78a03e2456` (갱신: 2026-06-01; 이전: `e0465fdc18bfeb67f9f114f7a37835269d35294d`): https://github.com/Yeachan-Heo/oh-my-codex/tree/ff17267baa3a69b7b3578c64edf8cc78a03e2456
- OMC architecture, hooks, team, MCP, persistent-mode, keyword-detector: `README.md`, `.claude-plugin/plugin.json`, `docs/HOOKS.md`, `docs/agents/model-compatibility.md`, `scripts/persistent-mode.mjs`, `scripts/pre-tool-enforcer.mjs`, `src/hooks/*`, `src/mcp/*`, `skills/omc-teams/SKILL.md`
- OMX architecture, plugin/setup split, hooks, team, config, MCP: `README.md`, `plugins/oh-my-codex/.codex-plugin/plugin.json`, `docs/codex-native-hooks.md`, `docs/plugin-bundle-ssot.md`, `docs/team-coordination-protocol.md`, `docs/ultragoal.md`, `src/config/generator.ts`, `src/auth/hotswap.ts`, `src/autopilot/fsm.ts`, `src/team/coordination-protocol.ts`, `src/agents/definitions.ts`, `prompts/scholastic.md`, `skills/ralplan/SKILL.md`, `skills/worker/SKILL.md`
