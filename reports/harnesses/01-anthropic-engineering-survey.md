...
scope: "Anthropic 공식 Engineering 인덱스와 site search로 확인한 /engineering 하위 agent, coding, harness, tool, eval 관련 문서"
sources:
...
upstream: []
last_ingested: 2026-05-21T00:00:00+09:00
kind: ditto-research
---

# Anthropic Engineering Blog 조사 및 검증

이 문서는 `https://www.anthropic.com/engineering` 하위 문서를 2026-05-21 기준으로 재검증한 결과다. 기존 문서는 15편 기준이었지만, 공식 Engineering 인덱스는 2026-04-23 postmortem까지 더 많은 문서를 노출한다. 추가 웹 검색으로 인덱스에 보이지 않지만 `/engineering/` 하위에 존재하는 `Building agents with the Claude Agent SDK`도 확인되어 분석 범위에 포함했다.

결론부터 말하면 Anthropic의 Engineering 글들은 하나의 메시지로 수렴한다. 에이전트는 똑똑한 모델 하나가 아니라, 컨텍스트 예산, 도구 표면, 검증 루프, 샌드박스, 세션 로그, 평가 하네스가 같이 움직이는 시스템이다. 그러니까 하네스가 중요하다. 놀랍게도, 접착제가 또 이겼다.
...
- 검증 한계: Anthropic 사이트는 동적 인덱스와 리다이렉트를 사용한다. 이 문서는 2026-05-21에 확인한 공개 웹 상태 기준이다.
...
DITTO에 대한 추론: 초기 버전은 거창한 planner/executor runtime보다 `model call -> tool call -> observation -> next model call`을 노출 가능한 형태로 두는 편이 낫다. Anthropic도 framework를 쓰더라도 underlying prompt와 response를 이해해야 한다고 경고한다. 그러니까 DITTO도 "마법 같은 autonomous engine"보다 "뭘 호출했고 왜 망했는지 볼 수 있는 harness"가 맞다.
...
### Managed Agents는 구현보다 interface를 안정화한다

`Scaling Managed Agents`는 harness가 모델 한계에 대한 가정을 인코딩하며, 그 가정은 모델 향상으로 stale해질 수 있다고 말한다. 그래서 session, harness, sandbox를 별도 interface로 분리한다. session은 append-only log, harness는 Claude 호출과 tool routing loop, sandbox는 code/file execution environment다. brain은 harness/model, hands는 sandbox/tools, session은 durable event log로 decouple된다. 근거: [Scaling Managed Agents](https://www.anthropic.com/engineering/managed-agents).

DITTO에 대한 추론: DITTO의 핵심 추상화는 특정 모델 버전의 버릇이 아니라 오래 버틸 interface여야 한다. 예를 들어 `SessionLog`, `ToolExecutor`, `Sandbox`, `ContextAssembler`, `PolicyGate`를 분리하면 모델이 context anxiety를 덜 보이는 날이 와도 코드를 덜 버리게 된다. 뭐, 언젠가 버리긴 하겠지만 덜 아깝다.
...
DITTO에 대한 추론: compaction은 같은 agent의 기억을 줄이는 기술이고, reset은 새 worker에게 업무 인수인계를 시키는 기술이다. DITTO는 둘을 하나의 "요약" 기능으로 뭉개면 안 된다. reset에는 다음 session이 독립적으로 시작해도 작업을 재현할 enough state가 필요하다.
...
DITTO에 대한 추론: DITTO의 세션 저장소는 "프롬프트에 넣은 마지막 요약"이 아니라 append-only 원장이어야 한다. 요약은 파생물이다. 원본 trace가 없으면 evaluation도 postmortem도 없다. 물론 디스크는 싸고 토큰은 비싸다. 이 정도는 기억해도 된다.
...
### Parallel Claudes는 단순한 file lock만으로도 움직인다
...
DITTO에 대한 추론: 병렬 agent orchestration은 처음부터 분산 시스템처럼 만들 필요는 없다. 파일 기반 lock, git commit, container isolation, role prompts만으로도 배울 수 있다. 다만 DITTO 제품화에는 task ownership, merge conflict policy, shared state schema, evaluator lane, budget control이 필요하다.

### Generator와 evaluator는 분리해야 한다

`Harness design for long-running application development`는 agent가 자기 산출물을 평가할 때 과하게 긍정적으로 판단하는 self-evaluation 문제를 지적한다. 해결책은 generator와 evaluator를 분리하고, evaluator에게 concrete criteria와 tools(예: Playwright)를 주는 것이다. 근거: [Harness design](https://www.anthropic.com/engineering/harness-design-long-running-apps).

DITTO에 대한 추론: DITTO에는 writer와 reviewer가 같은 context/goal을 공유하지 않는 독립 검증 lane이 필요하다. 특히 frontend, UX, long-running app build처럼 binary test만으로 부족한 영역에서는 evaluator가 실제 앱을 조작하고 screenshot/log/test evidence를 남겨야 한다.
...
### 2. Context는 budget이고 artifact는 storage다

근거: `Effective context engineering`, `Code execution with MCP`, `Managed Agents`.

큰 파일과 tool results는 context가 아니라 artifact로 둔다. context에는 pointer, summary, selected slice만 들어가야 한다.

### 3. Session log는 append-only 원장이어야 한다

근거: `Managed Agents`, `Effective harnesses`.

요약/compaction은 재생성 가능한 view다. 원본 event stream을 잃으면 postmortem과 eval이 불가능하다.

### 4. 도구는 agent가 이해하는 단위로 다시 설계한다
...
### 9. 병렬화는 compression 장치다

근거: `Multi-agent research`, `Building a C compiler`.

parallel agents는 더 많은 context를 parent에 넣기 위한 방법이 아니라, 각자 context를 태우고 작은 evidence summary를 돌려주는 방법이다.

### 10. Postmortem이 없는 harness는 곧 미신이 된다
...
| 2026-02 | infra noise와 parallel agents가 전면에 나온다. | 실행 환경과 병렬 coordination을 기록하라. |
| 2026-03 | evaluator 분리, auto mode, eval awareness가 중요해진다. | self-eval과 manual approval만으로는 부족하다. |
| 2026-04 | managed agents와 postmortem은 interface 안정성과 harness regression을 강조한다. | 구현은 바뀌어도 session/harness/sandbox 계약은 오래가야 한다. |

## DITTO 보완 계획

1. `SessionLog`를 append-only event stream으로 설계한다.
   - 근거: Managed Agents는 session을 Claude context window와 분리하고 durable event log로 둔다.

2. `ContextAssembler`를 별도 모듈로 둔다.
   - 근거: Effective context engineering은 token set selection을 agent 성능의 핵심 문제로 본다.

3. `ToolRegistry`는 search/defer/examples를 지원한다.
   - 근거: Advanced tool use는 tool definitions가 많아질 때 deferred loading과 examples가 필요하다고 한다.

4. MCP/외부 도구는 code API 또는 sandboxed programmatic execution 경로를 제공한다.
   - 근거: Code execution with MCP는 direct tool call보다 code API가 context 효율적임을 보여준다.

5. Skills는 metadata-only preload와 on-demand body loading을 기본으로 한다.
   - 근거: Agent Skills의 progressive disclosure 구조.

6. Long-running task에는 initializer/coding/evaluator roles를 분리한다.
   - 근거: Effective harnesses의 initializer/coding split, Harness design의 planner/generator/evaluator split.

7. Completion contract를 "검증 evidence" 중심으로 정의한다.
   - 근거: Claude Code best practices는 tests/screenshots/expected outputs를 최고 leverage로 본다.

8. Eval report에는 infra config를 필수 필드로 둔다.
   - 근거: Infrastructure noise가 resource config만으로 6%p 차이를 만들 수 있음을 보여준다.

9. Permission system은 manual prompt, sandbox, auto classifier를 조합한다.
   - 근거: sandboxing과 auto mode 문서는 각각 permission fatigue와 isolation 한계를 보완한다.

10. Prompt/tool/policy 변경에는 ablation과 rollback plan을 둔다.
    - 근거: April 23 postmortem은 system prompt 한 줄과 reasoning effort default도 coding quality를 바꿀 수 있음을 보여준다.

## 근거 목록

- [Engineering index](https://www.anthropic.com/engineering): 2026-05-21 기준 공식 Engineering 목록과 featured post 확인.
...
1. Append-only `SessionLog`와 구조화된 handoff를 ditto의 기본 실행 단위로 둔다.
   - 적용할 기능/가치: PURPOSE.md의 "모든 액션에는 감사 기록이 누적된다", "주요 결정 및 변경사항 영속화", "Context Rot 이슈 해결", "장기간 실행되는 작업"에 직접 대응한다.
   - 적용 방식: 세션 로그를 context window와 분리된 append-only event stream으로 저장하고, context에는 필요한 event slice와 요약만 넣는다. 장기 작업은 `features.json`, `progress.md`, git commit log, runbook, session trace 같은 handoff artifact로 다음 세션이 독립적으로 이어받게 한다.
   ...
   - 리스크/선행 조건: 로그 schema와 보존 정책이 먼저 필요하다. 요약이 원본을 대체하면 postmortem과 eval이 불가능하므로 summary는 파생물로만 취급해야 한다.
   - 근거: 보고서의 `Session log는 Claude의 context window가 아니다`, `Long-running harness는 handoff artifact가 핵심이다`, `Context reset은 compaction과 다르다` 섹션은 각각 `Scaling Managed Agents`, `Effective harnesses for long-running agents`, `Harness design for long-running application development`를 근거로 session/handoff/context reset 분리를 설명한다.
...
5. deep module 원칙에 맞춰 오래 버틸 harness interface를 우선 설계한다.
   ...
   - 근거: 보고서의 `단순한 agent loop에서 출발한다`, `Agent SDK는 "컴퓨터를 주는 것"을 중심으로 확장된다`, `Managed Agents는 구현보다 interface를 안정화한다`, `DITTO 설계 원칙으로 재정리` 섹션은 `Building effective agents`, `Building agents with the Claude Agent SDK`, `Scaling Managed Agents`를 근거로 단순 루프와 session/harness/sandbox interface 분리를 제시한다.
