...
상세 의역 (2026-06-01 갱신): 이 문서는 Managed Agents를 실제 Claude API에서 쓰는 방법을 설명하는 제품 문서다. 2026-06-01 기준으로 핵심 개념은 네 가지로 정리된다: **Agent** (모델, system prompt, tools, MCP server, skills), **Environment** (세션이 실행되는 환경 — Anthropic 관리형 클라우드 sandbox 또는 self-hosted sandbox), **Session** (특정 작업을 수행하는 running agent instance), **Events** (user turn, tool result, status update 등 애플리케이션과 agent 간 메시지). 문서는 전체 베타(`managed-agents-2026-04-01` 헤더 필수)이며, ZDR(Zero Data Retention) 및 HIPAA BAA 적용 대상이 아니라는 제약을 명시한다. [MAD]

**2026년 5월 신규 출시 (출처: https://claude.com/blog/new-in-claude-managed-agents, 2026-05-06):**

- **Self-hosted sandboxes (공개 베타)**: Anthropic이 orchestration을 유지하면서 tool 실행 환경만 사용자 인프라로 이동. 보안·규정·데이터 주권 요구사항에 대응. Cloudflare, Daytona, Modal, Vercel 연동 가이드 제공.
- **Outcomes (공개 베타)**: 개발자가 rubric을 작성하면 별도 grader agent가 자체 context window에서 산출물을 평가. HDL의 evaluator 분리 원칙이 제품화된 형태. 문서 생성 작업에서 +8.4%, 프레젠테이션 생성에서 +10.1% 성공률 향상 보고.
- **Dreaming (연구 프리뷰, `dreaming-2026-04-21` 헤더 추가 필요)**: 과거 session transcript를 읽어 memory store를 정리·재구성하는 비동기 파이프라인. 원본 store는 수정하지 않고 새 output store를 생성. memory 기능(`/docs/en/managed-agents/memory`)의 상위 계층.
- **Multiagent orchestration (공개 베타)**: 한 lead agent가 작업을 쪼개 전문화된 subagent에게 위임. subagent는 공유 filesystem과 persistent event history에 접근하며 병렬 실행 가능. Claude Console에서 전체 실행 흐름 모니터링 지원.
- **Webhooks**: session 완료 시 개발자 endpoint 호출 알림.

MA와의 연결: MA가 철학과 아키텍처를 설명한다면, MAD는 그 개념을 API 표면으로 내린다. session/harness/sandbox 분리가 실제 제품에서 별도 객체와 event로 드러난다. 2026-05-06 출시에서 Outcomes는 HDL의 evaluator 분리를, Dreaming은 ECE의 memory-outside-context-window 패턴을, self-hosted sandboxes는 MA의 sandbox disposability 원칙을 각각 제품화한 사례로 볼 수 있다.
...
- **신규**: Outcomes의 rubric 기반 grader 패턴은 DITTO `Evaluator` 설계의 실제 운영 사례다. generator와 evaluator를 별도 context window로 분리한다는 원칙이 Anthropic 제품 수준에서 검증됐다.
- **신규**: self-hosted sandbox는 DITTO의 `Sandbox` 인터페이스가 Anthropic 관리형과 자체 호스팅 양쪽을 추상화해야 함을 시사한다. 지금 Anthropic 관리형만 전제한 구현은 나중에 교체가 어려워질 수 있다.
...
검토 기준일: 2026-06-01. 각 출처를 직접 fetch하여 확인한 결과다.

### 변경 없음

| 출처 | 확인 결과 |
|---|---|
| [MA] Scaling Managed Agents (https://www.anthropic.com/engineering/managed-agents) | 2026-04-08 게재 이후 수정 없음 |
| [EHA] Effective harnesses for long-running agents (https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents) | 2025-11-26 게재 이후 수정 없음 |
| [HDL] Harness design for long-running application development (https://www.anthropic.com/engineering/harness-design-long-running-apps) | 2026-03-24 게재 이후 수정 없음 |
| [ECE] Effective context engineering (https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents) | 2025-09-29 게재 이후 수정 없음 |
| [BEA] Building effective agents (https://www.anthropic.com/engineering/building-effective-agents) | 2024-12-19 게재 이후 수정 없음 |
| [BL] The Bitter Lesson | 정적 외부 글, 변경 없음 |
| [TAOUP] The Art of Unix Programming | 정적 외부 글, 변경 없음 |
| [PVC] The History of Pets vs Cattle | 정적 외부 글, 변경 없음 |
| [RLM] arXiv:2512.24601 | 논문, 변경 없음 |

### 중요 변경: MAD (Claude Managed Agents docs)

출처: https://platform.claude.com/docs/en/managed-agents/overview — 내용 대폭 확장 확인.

2026년 4월–5월 사이에 다음 기능이 추가됐다. 이 문서의 3.1절 MAD 주석을 그에 맞게 갱신했다.

**1. Environment 개념 분리 (구조 변경)**
기존 annotation은 "agent environment와 session event"를 핵심 개념으로 기술했다. 현재 문서는 Agent / Environment / Session / Events 네 가지를 명시적으로 구분한다. Environment는 Anthropic 관리형 클라우드 sandbox와 self-hosted sandbox 중 선택 가능한 별도 객체다. MA의 "brain과 hands 분리" 원칙이 API 표면으로 내려온 것으로 볼 수 있다.

**2. Self-hosted sandboxes 공개 베타** (https://platform.claude.com/docs/en/managed-agents/self-hosted-sandboxes)
Anthropic이 orchestration을 유지하면서 tool 실행만 사용자 인프라로 이동. Cloudflare, Daytona, Modal, Vercel 연동 가이드 공개. 단, 현재 memory 기능은 self-hosted 환경에서 미지원.

**3. Outcomes 공개 베타**
rubric 기반 별도 grader agent가 독립 context에서 산출물 평가. HDL의 evaluator 분리 원칙과 직접 대응.

**4. Dreaming 연구 프리뷰** (https://platform.claude.com/docs/en/managed-agents/dreams)
과거 session transcript를 읽어 memory store를 비동기적으로 재구성. 원본 store 불변. `dreaming-2026-04-21` 베타 헤더 별도 필요.

**5. Multiagent orchestration 공개 베타**
lead agent가 전문 subagent에게 작업 위임, 공유 filesystem, persistent event history 공유. EHA의 initializer/coding split 원칙이 제품화된 형태.

**6. Webhooks, Rate limits, ZDR 제약 명시**
session 완료 알림 webhook 추가. 조직당 create 300 req/min, read 600 req/min 제한. Managed Agents는 stateful 특성상 ZDR 및 HIPAA BAA 적용 불가 명시.

**DITTO 함의 (신규 발견)**
- Outcomes는 DITTO `Evaluator` 설계의 운영 검증 사례다. generator와 evaluator를 별도 context window로 분리하는 원칙이 Anthropic 제품 수준에서 채택됐다.
- self-hosted sandbox 존재는 DITTO의 `Sandbox` 인터페이스가 provider-agnostic해야 함을 뒷받침한다.
- Dreaming은 DITTO session log에서 memory를 파생하는 방식의 참조 구현으로 볼 수 있다. DITTO가 session event stream을 원본으로 보존해야 하는 이유(section 2.4, 2.8)가 실제 제품에서 재확인됐다.

### 검증 상태 (2026-06-01 갱신)

- RLM (arXiv:2512.24601): **검증됨** — 실존 논문 "Recursive Language Models"(Alex L. Zhang, Tim Kraska, Omar Khattab), 제출 2025-12-31, 최신 v3 2026-05-11. arXiv ID·저자·제목 확인 완료(출처: `arxiv.org/abs/2512.24601`).
- Dreaming/Outcomes/Multiagent: research preview / 공개 베타 단계 — 이는 검증 실패가 아니라 **제품 성숙도에 따른 forward-looking 캐비엇**이다(현 시점 상태는 §3.1에 반영됨, 향후 GA 시 성능·제약 재확인 필요). 이 한 줄만 standing re-check 대상으로 남긴다.
