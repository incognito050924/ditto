...
- 로컬 분석 경로: `/tmp/lazycodex-investigation.TXhRmD`
- LazyCodex 기준 커밋: `7bbf9d78a532a57aabf23fb213d214b82eeca892`
- `src` 서브모듈: `https://github.com/code-yeongyu/oh-my-openagent.git` / `65715d1c2c35e27ccf2195ef688b0909dddb403c`
- `src/packages/lsp-tools-mcp` 서브모듈: `d1ff1681c1f558e062ac33fad4d835baaa7b5edf`
...
- `SessionStart`: rules, telemetry, auto-update
- `UserPromptSubmit`: rules, ultrawork, ulw-loop steering
- `PreToolUse`: Bash reminder, goal budget guard
- `PostToolUse`: comment checker, LSP, rules
- `PostCompact`: cache reset
- `Stop`, `SubagentStop`: start-work continuation

근거: `plugins/omo/hooks/hooks.json:2-179`.
...
근거: `src/packages/ast-grep-core/src/sg-compact-json-output.ts:1-120`, `src/packages/ast-grep-core/src/language-support.ts:1-38`.
...
AST-grep은 `.mcp.json`으로 Codex에 노출된다. ultrawork directive는 구조 검색과 codemod에는 `ast_grep_search`를 쓰라고 명시한다.

근거: `plugins/omo/.mcp.json:3-8`, `plugins/omo/components/ultrawork/directive.md:185-186`.
...
이번 보고서 작성 전 다음 읽기 전용 검증을 다시 실행했다.
