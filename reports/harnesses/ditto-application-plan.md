...
| 항상 켜진 multi-model council | 비용과 지연이 크고 작은 작업에는 과하다. | opt-in expensive review command로 둔다. |
...
목표: host별 지침과 설정 drift를 실행 전에 드러낸다.
...
- `AGENTS.md`와 host별 projection 파일의 drift
- `CLAUDE.md`, `.claude/settings.json`, `.codex/config.toml` 존재 여부와 managed block
- dangerous mode, network-on, secrets-read 가능성
- MCP server/tool inventory와 side effect label
- skill/agent/command manifest와 실제 파일의 일치

검증:

- 정상 fixture와 의도적으로 drift가 있는 fixture를 모두 테스트한다.
- doctor는 human output과 JSON output, exit code를 모두 제공한다.
...
목표: raw provider 호출을 DITTO run으로 감싼다.
...
- profile, cwd, git head, dirty before/after
- prompt/context packet path
- stdout/stderr, diff, exit status
...
| plan-check | read-only | coverage/conflict/risk verdict |
...
- Stop: completion contract 검사와 handoff 생성 유도
...
### v0.1: Evidence-bearing work ledger

포함:

- `.ditto/work-items`와 `.ditto/runs` schema
- `work start/status/handoff`
- `run record`
- completion contract
...
### v0.2: Doctor and instruction bridge
...
- `doctor mcp`
- `surface/inventory`
- host projection drift check

완료 기준:

- Codex/Claude Code가 읽는 지침, permission, MCP, skill 표면의 drift가 human/json 결과와 exit code로 드러난다.

### v0.3: Provider wrapper and context packet

포함:

- Codex wrapper
- Claude Code wrapper
- read-only/workspace-write/reviewer profile
- `context build`
- stdout/stderr/diff/verification capture
...
1. `.ditto`를 repo에 commit할 기본 범위: work item/handoff schema는 commit 후보지만 logs/screenshots는 보존 정책이 필요하다.
...
6. multi-model 검토 비용 정책: opt-in command의 budget cap, timeout, partial result contract가 필요하다.

## 8. 다음 작업

가장 작은 다음 작업은 v0.1을 구현하기 전에 `schemas/` 또는 `docs/contracts/` 위치를 정하고, work item/run/completion/reviewer contract fixture를 만드는 것이다. 그 다음에야 wrapper, doctor, hook을 붙여도 상태와 evidence가 흩어지지 않는다.
