...
작성: 2026-06-02 · branch `main` · HEAD `f5ae279`
...
## 끝난 것 (전부 main에 커밋, 758 tests pass · lint 0 error)
...
- **작은 고정 2건** `wi_260602f93` — **nextNode terminal surfacing 정확화**: (1) **escalate blocked surfacing**(A-2) — ready 0 & 미terminal에서 running 없고 `blocked` 노드가 있으면 새 `action:'blocked'`로 `blocked_node_ids` + decisions 로그 reason을 surface(transient `waiting`과 구분). (2) **§6.8 done disposition** — `done`에 `all_passed:boolean` 추가 + reason이 완료 판정 필요(graph done ≠ AC closed) 명시. **done은 work-item AC를 auto-close 안 함**(증거 기반 §6.8 유지, false-green 차단 — dogfood로 done 직후 AC verdict unverified 입증). CLI human 출력·`skills/autopilot/SKILL.md` action 목록 일관 반영. 기존 action 보존. commits `6815e57 f5ae279`.
...
작은 고정 2건까지 완료(§6.8 surfacing은 done이 AC를 auto-close 하지 않게 *정확히* 고침 — auto-close는 증거 기반 원칙 위반이라 의도적으로 안 함). 다음 증분 후보(추천 순):
1. **done→completion 자동 어셈블** `autopilot complete` — 수집된 노드 evidence를 AC에 매핑해 completion 계약을 *미검증 verdict로* 어셈블(verifier가 확정). 매 dogfood마다 수동 `buildCompletion` 스크립트 작성하는 마찰을 없앰. 증거 기반 유지(auto-pass 아님). 추천.
2. **cleanup 배선** — 결정적 드라이버 스텝 + 비가역 git 승인 게이트(마지막 [VERIFY] kind, 다른 형태).
3. **security 노드 forward 재확장**(§2.4) — A-2 게이트를 `review`에서 `security`까지 확장(작음).
4. **planner/owner subagent 실제 spawn 통합 경로** — 엔진이 노드에서 owner를 spawn해 진짜 LLM 생성/분석을 측정(부트스트랩 역설 완전 해소). 큰 작업.

## Known issues (조치 대기, follow-up)
- **done→completion 수동 어셈블**: `done`은 이제 disposition(`all_passed`)을 정확히 surface하지만, 거기서 completion 계약을 자동 어셈블하진 않음 — 매 증분 수동 `buildCompletion`. (위 다음 후보 #1.)
- **security 노드 forward 재확장 미적용**([VERIFY]): A-2 게이트가 `review` kind만 — §2.4는 `security`도 포함(현재 security `has_findings`는 무시). 위 후보 #3.
- **`ditto work handoff` changed_files 과대산출**: 오래된 base ref로 무관 파일 수십 개를 나열. 우회 = `--base <tight-ref>`. 이 핸드오프 문서는 그래서 손으로 씀.
...
- 코드 seam: `src/schemas/autopilot.ts`(nodeKind/nodeOwner enum — [VERIFY] kind/owner 포함), `src/core/autopilot-graph.ts`(addNodes 무결성·proposalsToNodes·KIND_TO_OWNER·buildInitialNodes), `src/core/autopilot-converge.ts`(forward 재확장 planner + `forwardRound`), `src/core/autopilot-loop.ts`(nextNode 종단 surfacing[blocked/done all_passed] + recordResult + A-3 승격 + A-2 forward 재확장 + `isMutatingNode`), `src/core/autopilot-dispatch.ts`(`buildDelegationPacket` + `OWNER_TOOLS` + `isMutatingOwner` + `PLANNER_GENERATE_DIRECTIVE`), `agents/*`(owner 계약: planner=graph generator, security-reviewer/refactorer/retrospective=[VERIFY] owner), `src/core/autopilot-bootstrap.ts`. **surface 변경 시 `bun run surfaces:gen` + `surface-inventory.plugin.test.ts`의 하드코딩 count 갱신.**
...
## 새 세션 첫 체크 + 금지
- 첫 체크: `git log --oneline -6` → `bun test`(758 green 기대) → 계약 §2.2/§2.4/§4.3/§6.8 재독 → 다음 증분 결정.
