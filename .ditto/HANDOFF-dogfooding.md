...
작성: 2026-06-02 · branch `main` · HEAD `fc08958`
...
## 끝난 것 (전부 main에 커밋, 748 tests pass · lint 0 error)
...
## 부트스트랩 역설 현재 상태
**부분 해소(메커니즘 2축 모두 live).** `addNodes`(A-1)가 두 live 호출자를 얻음: (1) A-3 = design 노드의 `generated_nodes` 자유 성장, (2) A-2 = review 노드 findings의 forward 수렴 라운드(budget 지배). 그래프가 씨앗을 넘어 *전진 성장* + *수렴 루프* 둘 다 가능. **아직 없는 것 = 지능**: LLM planner가 root_goal/AC를 보고 *무슨 노드를* 만들지/리뷰가 *무슨 findings를* 낼지 결정하는 부분. 기본 bootstrap은 여전히 3노드를 seed(관찰 동작 불변), `has_findings`도 호출자(드라이버)가 공급.

## 다음 결정 (열려 있음 — user-owned)
A-2 live 배선까지 완료. 다음 증분 후보(추천 순):
1. **planner 지능** — LLM이 root_goal/AC로 노드 서브그래프를 *생성*. A-3/A-2로 메커니즘이 다 깔려 가장 자연스러움. (수동 TDD 한계 주의: 진짜 LLM 생성은 엔진이 owner subagent를 spawn해야 실측 가능 — 결정론 부분/계약부터.)
2. **작은 고정 2건** (아래 known issues) — dogfooding 루프 충실도.
3. **[VERIFY] owner 제작**(security/test/refactor/retro/cleanup) — planner 지능 선행 필요.
...
- 코드 seam: `src/core/autopilot-graph.ts`(addNodes 무결성·proposalsToNodes·buildInitialNodes), `src/core/autopilot-converge.ts`(forward 재확장 planner + `forwardRound` 도출), `src/core/autopilot-loop.ts`(nextNode/recordResult + A-3 generated_nodes 승격 + A-2 forward 재확장 배선), `src/core/autopilot-bootstrap.ts`.
...
## 새 세션 첫 체크 + 금지
- 첫 체크: `git log --oneline -6` → `bun test`(748 green 기대) → 계약 §2.4/§4.3 재독 → 다음 증분 결정.
