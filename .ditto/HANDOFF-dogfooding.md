...
작성: 2026-06-02 · branch `main` · HEAD `be7fdda` (push 후 origin/main 최신)
...
## 끝난 것 (전부 main에 커밋·push, 766 tests pass · lint 0 error · tsc 무회귀)
...
## 다음 결정 (열려 있음 — user-owned, 추천 순)
1. **changed_files 자동 수집** — `autopilot complete`가 work-item.changed_files를 읽는데 수동 dogfood에선 비어 있어 매번 핀이 필요. implementer 노드가 work-item.changed_files를 갱신하게(또는 `complete --changed-files`). 작음.
2. **cleanup 배선** — 결정적 드라이버 스텝 + 비가역 git 승인 게이트(마지막 [VERIFY] kind, 다른 형태).
3. **security 노드 forward 재확장**(§2.4) — A-2 게이트를 `review`→`security`까지(작음, 현재 security `has_findings`는 무시됨).
4. **owner subagent 실제 spawn 통합** — 부트스트랩 역설 완전 해소. 큰 작업.
...
- 코드 seam: `src/schemas/autopilot.ts`(nodeKind/nodeOwner), `autopilot-graph.ts`(addNodes·proposalsToNodes·KIND_TO_OWNER), `autopilot-converge.ts`(forward 재확장·forwardRound), `autopilot-loop.ts`(nextNode 종단 surfacing·recordResult·승격·isMutatingNode), `autopilot-dispatch.ts`(buildDelegationPacket·OWNER_TOOLS·isMutatingOwner·PLANNER_GENERATE_DIRECTIVE), `autopilot-complete.ts`(done→completion 어셈블), `completion-store.ts`(buildCompletion), `agents/*`, `skills/autopilot/SKILL.md`.
