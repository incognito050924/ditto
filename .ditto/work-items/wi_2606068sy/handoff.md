...
- ✅ **increment 6**: 통합 실증 (커밋 대기 — boxwood는 사용자 레포라 ditto repo에 코드 변경 없음, 이 핸드오프만 갱신) — **클린 boxwood 레포(`java/workspace/boxwood-domain-model-java`, Kotlin/Gradle)에 `scripts/install.sh install --target <boxwood>` 실제 실행**: register(실글로벌 settings, 백업됨)·build·place(`~/.local/bin/ditto`, `which -a ditto` 첫 항목=우리것)·codeql reuse·playwright reuse·init·allowlist 전부 ok. `ditto doctor`(capability/permissions) 타겟에서 동작. **autopilot e2e 완주**: work start(repo_root=boxwood)→deep-interview(threshold0)+finalize(3노드 부트스트랩 N1 design/N2 implement/N3 verify)→next-node 디스패치→record-result(N1 plan, N2 docs/DITTO-SMOKE.md 생성+file 증거, N3 독립검증 command+file 증거)→complete=**evidence-gated `final_verdict: pass`**(ac-1 verdict=pass, 증거 3개). §1-C의 "DITTO가 실타겟에 구동된 적 없음" 해소. **한계**: 이 세션이 ditto repo에 루트돼 있어 boxwood로 spawn한 subagent는 scope-out 훅에 막힘 → 충실한 subagent 루프는 boxwood-루트 세션 필요(엔진/CLI 루프 자체는 실증됨; subagent spawn은 self-host ed075a1에서 기실증).
  - **검증 후 발자국 전부 되돌림(2026-06-06, 사용자 요청)**: `install.sh uninstall` + boxwood `{.ditto,.claude}` 제거 → 글로벌 `~/.claude/settings.json` 검증 전과 byte-identical, `~/.local/bin/ditto` 제거, boxwood git status clean(내 커밋 0건 — 애초에 boxwood엔 커밋 안 함). **현재 사용자 환경에 설치 잔재 없음.** 설치 스크립트 코드는 ditto repo에 커밋돼 재설치 언제든 가능.
...
### 잔여 상태 (사용자 환경) — 없음
inc6 검증이 글로벌/타겟을 잠시 건드렸으나 **사용자 요청으로 전부 되돌림(2026-06-06)**: uninstall(register/place/allowlist 역행) + boxwood `{.ditto,.claude}` 수동 제거 + 오늘자 백업 정리. 현재: 글로벌 `~/.claude/settings.json` 검증 전과 byte-identical, `~/.local/bin/ditto` 없음, boxwood git status clean. **설치 잔재 0.** 재설치는 `DITTO_HOME=<repo> bash scripts/install.sh install --target <target>`.
