...
> "이 핸드오프(`.ditto/work-items/wi_2606068sy/handoff.md`) 읽고, DITTO 설치 스크립트(wi_2606068sy) **increment 4(CodeQL 자동 설치)** 부터 이어서 진행해. 검증은 self-host 말고 임시 clean 타겟에서."

increment 4 메모: 오케스트레이터(install-plugin.mjs)에 CodeQL 설치 단계 추가 — 플랫폼별 다운로드 + `CODEQL_BIN`/PATH 설정. graceful(실패 시 정확한 수동 명령 안내, install 자체는 계속). 탐지 로직 기준: `src/core/codeql/doctor.ts:227`(CODEQL_BIN→which→gh ext). build/place와 동일하게 step 함수로 추가하고 `{ok,message}` 반환. inc5(playwright)도 같은 패턴. 검증 시 CodeQL은 대용량 다운로드라 graceful-skip 경로(이미 있으면 재사용, 없으면 안내)를 우선 실증.
