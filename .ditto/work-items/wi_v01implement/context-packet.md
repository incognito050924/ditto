...
## Relevant files
- 골격(수정 대상): `src/cli/commands/work.ts`, `src/cli/commands/run.ts`, `src/cli/commands/verify.ts`, `src/cli/util.ts`
- core 신규: `src/core/{fs,id,work-item-store,run-store,evidence-store}.ts`
- schema(읽기만): `src/schemas/`
- 자체 검증 테스트 신규: `tests/schemas/repo-self-validation.test.ts`
