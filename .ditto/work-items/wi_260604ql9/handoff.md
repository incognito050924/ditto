...
8. **PreToolUse forbidden_scope 집행** — **집행 코어 완료(wi_260605scp, §7-2)**: resolveScopeRef(path/glob/layer/public_surface) + ChangeContractStore + PreToolUse 차단(exit 2). symbol kind·ICL 생성 CLI·Change Map 렌더러는 후속. §5는 해소됨.
...
### 7-2. 후속 thread (2026-06-05): forbidden_scope 집행 (wi_260605scp, **status=done, final_verdict=pass**)
§2 #8 / §5 공백(계약에 forbidden_scope 적어도 강제 안 됨)을 **집행 코어**로 메움. 미커밋(워킹트리에 있음).
- **신규**: `src/acg/scope/resolve.ts`(scopeRefMatches/matchForbiddenScope — path/glob/layer/public_surface 경로 매처, globToRegExp·pathToLayer 재사용), `src/core/change-contract-store.ts`(`.ditto/work-items/<wi>/change-contract.json` read/write).
- **배선**: `src/hooks/pre-tool-use.ts`에 (d) forbidden_scope 집행 추가 — Write/Edit/MultiEdit의 file_path가 현재 work item 계약의 forbidden_scope에 들면 exit 2. session_id→SessionPointerStore→work_item_id→ChangeContractStore→archSpec(`.ditto/architecture-spec.json` optional). handler를 **async로 전환**(기존 테스트는 이미 `await call(...)` 패턴이라 회귀 0). 모든 전제 부재는 ALLOW(fail-open).
- 검증: 단위(scope-resolve·change-contract-store) + 훅(block/allow/fail-open) + 전체 1020 pass/0 fail, biome clean.
- **범위 밖(후속)**: ① ~~symbol kind~~ → **완료(wi_260605s5k)**. ② ~~ChangeContract 생성·저장 파이프라인(ICL CLI)~~ → **#8c 완료(wi_260605cgen)**: `ditto change-contract`가 compileIcl→ChangeContractStore.write. ③ ~~archSpec 관례 위치~~ → **완료(wi_260605s5k)**: `architecture propose --write`. ④ Change Map 렌더러(후속). ⑤ fitness 저장·executed/llm_judged provider(#4).

### 7-3. 후속 thread (2026-06-05): forbidden_scope 5 kind 완성 (wi_260605s5k, **status=done, final_verdict=pass**)
#8b(symbol) + archSpec 관례 위치를 묶어 forbidden_scope 집행을 **path/glob/layer/public_surface/symbol 5종 전부** 끝까지 동작하게 함. 미커밋(워킹트리).
- **symbol**: `src/core/codeql/relations.ts`에 `SYMBOL_DECL_QUERY_JS`+`resolveSymbolDeclFiles`(declFile 제한 없이 이름으로 선언 파일). `src/acg/scope/symbol-expand.ts`의 `expandForbiddenSymbols`가 change-contract 저장 시점에 symbol kind를 선언 path로 치환(repo-relative 환산, symbol 있을 때만 CodeQL 1회). change-contract CLI에 배선. 실측: `forbid symbol parseSarif` → `path src/core/codeql/sarif.ts`.
- **archSpec**: `architecture propose --write` → `.ditto/architecture-spec.json`. pre-tool-use.loadArchSpec이 이미 그 경로를 읽으므로 layer/public_surface forbidden이 집행됨(chain 테스트 2 pass).
- **gotcha(신규)**: CodeQL `getRelativePath()`는 **source-root 기준**이다. PreToolUse는 repo-relative로 매칭하므로 symbol 해소 path를 `relative(repoRoot, join(sourceRoot, f))`로 환산해야 한다(안 하면 `core/...` vs `src/core/...` 불일치로 집행 실패 — 이번에 한 번 걸림).
- **잔여(후속)**: architecture-spec.json은 produced_by=agent candidate(비준 전, DITTO repo 미커밋). ~~fitness 저장·provider(#4)~~ → **fitness 저장 완료(wi_260605fit, §7-4)**. Change Map 렌더러 미착수.
- **신규 gotcha**: completion.json `evidence.kind`는 command/file/artifact/url/note (test/build/log 아님 — 이번에 한 번 걸림). work item의 evidence(ref/note)와 completion의 evidence(kind/summary)는 형식이 다름.

### 7-4. 후속 thread (2026-06-05): fitness 저장 + deterministic 전 사슬 (wi_260605fit, **status=done, final_verdict=pass**)
#4 중 **진짜 공백은 provider가 아니라 fitness 저장**이었음(deterministic provider·AssuranceSnapshot→stop 게이트는 이미 동작, change-contract가 fitness를 저장 안 해 죽어 있었음 — forbidden_scope와 동형). 미커밋(워킹트리).
- **신규**: `src/core/fitness-function-store.ts`(`.ditto/work-items/<wi>/fitness-functions.json`, AcgFitnessFunction[] read/write).
- **배선**: change-contract CLI가 compileIcl fitnessFunctions를 store.write. fitness CLI `--from`을 optional로 → 없으면 work item store 자동 로드(`ditto fitness run --work-item <wi>`만으로).
- 전 사슬 실측: deterministic fitness(`check: cmd "true"`) ICL → change-contract 저장 → fitness run 자동 로드 → `assurance-snapshot.json`(outcome pass). stop 게이트는 그 snapshot을 이미 읽음.
- **범위 밖(후속)**: executed/llm_judged provider(ditto 직접 LLM 금지 → 에이전트 판정 주입 인터페이스), fitness run 자동 트리거(autopilot/stop 배선), ICL executed mode 생성(현재 cmd/query→deterministic, judge→llm_judged만).
- ACG 거버넌스 양축(frame=forbidden_scope, assurance=fitness)이 둘 다 ICL→저장→집행/실행 전 사슬로 동작하게 됨.

### 7-5. 후속 thread (2026-06-05): fitness 자동 트리거 (wi_260605fat, **status=done, final_verdict=pass**)
fitness run 수동 호출을 없애고 **stop 훅이 완료 게이트 직전에 자동 평가**하게 함(사용자 결정: stop 자동 실행). 미커밋(워킹트리).
- **(구조적)** `commandProvider`를 `src/cli/commands/fitness.ts` → `src/acg/fitness/command-provider.ts`로 추출(동작 불변). fitness CLI와 stop 훅이 공유.
- **(동작적)** `src/hooks/stop.ts` `maybeRunFitness`: dir 확정 후 assurance read 전에 — FitnessFunctionStore.read 있으면 `runFitness(commandProvider)` → `assurance-snapshot.json` write → 기존 `assuranceSnapshotForcesContinuation` 게이트가 최신 snapshot을 읽음. 정의 없음·실행 에러는 try/catch **fail-open**. stale 없음(매 stop 최신 평가). export해 단위 테스트.
- 검증: stop-fitness.test.ts 3 pass(true→pass·echo violation(block)→fail·미정의→미작성), 기존 stop.test.ts 40 pass, 전체 1029 pass/0 fail.
- **잔여(후속)**: llm_judged/executed provider는 여전히 skip(자동 트리거해도 평가 안 됨). stop 훅이 매 완료 시도마다 shell 실행(8-iteration guard 내). stopHandler 통합 e2e는 maybeRunFitness 단위+기존 게이트 회귀로 갈음.
- **마일스톤**: ACG 거버넌스가 ICL→저장→(편집 시 PreToolUse 차단)+(완료 시 stop 자동 fitness 평가)로 **양축 자동화** 완성. 다음 후보: #7 boxwood 정식 바인딩(클론 전제), llm_judged/executed provider, #5 YAML, Change Map 렌더러.
