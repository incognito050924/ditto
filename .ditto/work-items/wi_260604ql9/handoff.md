...
- **범위 밖(후속)**: ① symbol kind — CodeQL impact가 매 PreToolUse에 부적합 → 계약 저장 시점 1회 해소해 path 변환. ② ~~ChangeContract 생성·저장 파이프라인(ICL CLI)~~ → **#8c 완료(wi_260605cgen)**: `ditto change-contract --work-item <wi> --file <a.icl>`가 compileIcl→ChangeContractStore.write. '생성→저장→집행' 전 사슬 e2e 통과(change-contract-chain.test.ts). fitness 저장·symbol 해소는 여전히 후속. ③ archSpec 관례 위치(`.ditto/architecture-spec.json`)가 없으면 layer/public_surface 미집행(path/glob만). ④ Change Map 렌더러.
