...
- **범위 밖(후속)**: ① symbol kind — CodeQL impact가 매 PreToolUse에 부적합 → 계약 저장 시점 1회 해소해 path 변환. ② ChangeContract 생성·저장 파이프라인(ICL CLI `ditto change-contract`) — 현재 계약은 store에 별도로 써야 집행됨. ③ archSpec 관례 위치(`.ditto/architecture-spec.json`)가 없으면 layer/public_surface 미집행(path/glob만). ④ Change Map 렌더러.
