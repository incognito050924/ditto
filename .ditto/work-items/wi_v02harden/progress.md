...
## 다음 동작
1. work-item.json status를 draft → in_progress로 갱신.
2. P-1(Codex TOML parser 교체 + ADR) 시작.
   - 외부 TOML lib 후보 선택 (`smol-toml` 1차 검토).
   - `src/core/hosts/shared.ts` 호출 지점 교체.
   - nested section/inline table 회귀 fixture 추가.
   - ADR-0001 보강 patch 또는 ADR-0003 신설을 같은 commit에.
