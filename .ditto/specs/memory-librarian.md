...
| ac-3 | evidence record가 `ditto memory propose-finding --work-item <id> --index <n>`로 MemoryEvent(confidence_kind=INFERRED)로 변환되어 propose→approve→query로 회수된다. **selector 정정**: EvidenceRecord에 안정 id가 없어 `--evidence-id`는 불가 — `--work-item`+`--index`로 레코드를 지정한다. 증거 출처(path:lines·command·sha256)는 `sources`(=`src_` memory source id 전용)가 아니라 event `text`에 결박한다. | propose→approve→query 왕복 출력 |
...
5. **증분 5 — 측정 + 게이트** [목표5, ac-5]. 재제안율 정의(후보 텍스트를 지배 ADR "기각된 대안"과 결정적 토큰 매칭), `src/core/memory-measure.ts` + `ditto memory measure`로 baseline 산출, 정의는 `reports/measurements/memory-curator-baseline.md`. measure-before-expand 게이트 데이터원. **빌드 발견**: 기각대안은 전용 `## 대안` 섹션에 구조화(20 ADR 중 18=90% 커버리지, 67건)되어 재제안율은 결정적으로 측정 가능하나, **불변식은 산문에 흩어져 전용 섹션이 없어** 위반율 분자를 결정적으로 못 낸다 → 불변식 위반율은 measure-before-expand로 **재보류**(ADR에 불변식 구조화 기재가 선행돼야 함). 검증: 측정 정의 + 1회 산출(완료).
