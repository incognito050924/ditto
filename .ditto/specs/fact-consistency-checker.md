...
> **소스**: tech-spec ac-9 게이트의 미해결 후보 (dialectic-1 obj-8 follow-up) + wi_260610hwd dogfooding
...
- **차단 게이트(fail-closed)로 즉시 출시** → 기각. 오탐률 미지 상태의 차단은 사용자 우회를 유발한다 — digest 과민 위험과 동형 선례(tech-spec 표면 설계 단계 결론). advisory-first + 계측 후 승격 결정 (§12 인터뷰 d-enforcement 가정).
