...
### D5 — 큐레이터 이원 능력: 조합형 + 생성형 (2026-06-18 후속 확정)

- 신규 시스템의 은유 무게중심은 **사서(보관·전달)가 아니라 큐레이터**다. 단순 검색·전달을 넘어 **(a) 자료를 엮어 구조(결정 계보·여정)를 구성하는 조합형**과 **(b) 근거에 결박된 답을 합성하는 생성형(`synthesis`)** 을 **둘 다 1급 능력**으로 가진다.
- 생성형 출력은 반드시 **출처 + 모르는 것(gap) 명시 + confidence 계급으로 사실과 분리**한다 — 합성이 검증 안 된 추론을 사실(EXTRACTED)로 굳히면 안 된다(세탁 방지, 비전의 할루시네이션 최소화 목적과 직결). gbrain의 "검색이 아니라 출처 붙은 답을 합성하되 gap을 명시"(`reports/research/gbrain-code-level-research.md`) 모델을 차용한다. 생성형은 D3의 그래프 투영 위에서 동작한다(D3 보완, 충돌 아님).
- 이 1급 생성형은 신규가 standalone이라 ditto memory의 advisory 자세(ADR-0013)에 구속되지 않기에 가능하다(D2: 신규는 자체 최적 설계). 반대로 ditto **내부** memory(ADR-0013)는 생성형을 INFERRED 채널로 한정해 advisory를 유지한다 — 두 갈래는 다른 자세를 의식적으로 택한다(`.ditto/specs/memory-librarian.md` §2).

> D5는 D1~D4(wi_260618o97 deep-interview, user_confirmation)와 달리 **2026-06-18 후속 대화**에서 추가 확정됐다. 시드 스펙(`reports/design/memory-librarian-external-seed-spec.md`) 잠긴결정 ⑥과 동일 내용이다.
...
- **D5**: 사용자 원본 비전(`memory-system.md`)이 "결정에 도움이 되는 답을 주는" 큐레이터를 요구하므로 단순 전달(사서)로는 부족하다. 생성형을 1급으로 두되 출처·gap·confidence 분리를 강제하면 할루시네이션 최소화(비전 목적)와 가치 합성이 양립한다. ditto 내부와 자세를 달리하는 것은 D2(신규=자체 설계)의 직접 귀결이다.
...
- 신규의 생성형 합성이 측정에서 환각을 줄이기는커녕 늘리면(합성 결론이 사실로 새는 세탁이 발생하면) → D5의 1급 생성형을 재검토하고 조합형 우선으로 후퇴한다.
