...
- 정합: `ditto knowledge adr-check` 가 형식 + 식별자 유니크성을 검사(fail-closed). CI/pre-merge 게이트로 권장. (index↔file 일관성 검사는 2026-06-25 철회 — 아래 amendment.)
- 기존 `ADR-NNNN-<slug>.md` 는 grandfather — rename 금지, 스키마가 legacy ∪ new 식별자를 동시 수용한다.

> **Amendment (2026-06-25, wi_2606247cx)** — 위 정합 항목에서 "index↔file 일관성" 검사(adr-check check 3)를 철회했다. 그 검사가 읽던 `knowledge.json` `decisions[]` 손-인덱스를 폐기했기 때문이다. 근거: ① 그 인덱스를 읽는 런타임 소비자가 0이었다 — CLAUDE.md 투영·memory 수집·`ditto memory query`·ADR-0020 가드레일은 전부 `adr/*.md` 파일(또는 그로부터 시드된 memory 이벤트 `adrGist`)을 직접 읽지 `decisions[]`를 읽지 않는다. ② check 3은 자기참조였다(인덱스가 담은 항목만 자기검증 — 25개 ADR 중 6개만 색인된 부분 drift). ③ `adr/*.md`가 결정 내용의 SoT인데 `decisions[]`가 그걸 손으로 재색인 → 헌장 §4-11(이중화=drift) 위반. 구조화된 결정 그래프(rationale·supersedes 계보·온톨로지)는 외부 메모리 프로젝트(ADR-0021 D5 "결정 계보")가 자체 설계로 담으므로 ditto에 부분 색인을 둘 이유가 사라졌다. ADR 식별자 정책 본체(파일명=불변 id, 형식·유니크성 검사)는 불변. 동반 변경: `knowledge-record` 스키마 `decisions[]` 필드·adr-check check 3·curator/knowledge-update 지침·glossary `decisions.md` 참조 제거.
