...
- 상태: accepted (결정 1~7 전부 코드 착지 — increment 1 AC↔oracle 코어 · increment 6 루프 규율 · increment 3 회고 측정·plan oracle 뷰·종료 disposition; K 카운터 per-AC 정교화 포함)
- 결정 일자: 2026-06-23 (proposed) · 2026-06-23 accepted 승격 (increment 1 wi_260623uap 착지) · 2026-06-24 increment 3·6 + K카운터 착지
- 결정자: hskim, claude (claude-opus-4-8)
- 관련: ADR-0023 (fused design 노드·far-field detect/propose/manual·비용/중복 거부 — 이 ADR은 그와 **정합(alignment)**, supersede 아님), ADR-0020 (결정-모순 가드레일·fail-closed — 투명성 원칙과 정합), ADR-0018 (옵셔널 도구 강등 — 정적 검증법 분석기 부재 시), ADR-0013 (완료 계약). 코드(권위, increment 1 착지 후 실제 닻): `src/schemas/work-item.ts`(`acOracle` — verification_method{dynamic_test|static_scan|soft_judgment} × maps_to × direction forward|backward, forward는 코드-포인터 maps_to 거부; `acceptanceCriterion.oracle` additive-optional), `src/core/autopilot-loop.ts`(design-close 경로가 recordResultPayload.ac_oracles를 work-item AC에 기록), `src/core/coverage-manager.ts`(`producePlanGate` 결정론 presence-CHECK만·순수 유지; `validateAcOracle`·`assertOracleFrozen`·`oraclesEqual` — 적대 검증+forward-AC frozen, design-close write 직전 배선), `src/core/autopilot-dispatch.ts`(`buildDelegationPacket` — AC 문장+oracle을 context.acceptance로 운반), `src/core/autopilot-complete.ts`(`deriveAcVerdicts`/`nodeVerdictFor` — AC↔oracle **닫힘 판정**), `src/core/gates.ts`(`oracleSatisfaction` — presence-gated·fail-closed·static_scan은 재스캔 evidenceRef 필요), `src/hooks/stop.ts:83`(`dialecticForcesContinuation` — **dialectic-OBJECTION 게이트**; ac-4 clause 2로 admissible objection의 file:line/path 닻 디스크 실재 검사 추가). 측정·규율 배선(착지): `src/core/retro-measure.ts`(회고 측정 ①outcome_floor·②process_health 분리), `src/core/autopilot-loop.ts`(design-close retro 노드 추가·`recordLoopTermination`·`sameOracleFailureCount` per-AC), `src/core/autopilot-driver.ts:113`(non-blocking retro 가드), `src/core/autopilot-store.ts`(`loop_terminated`·`disposition`·`criterion_ids`), `src/cli/commands/autopilot.ts:1021`(결정 5 oracle 뷰). 상류 측정 재료(후속): `src/core/coverage-feedback.ts`·`src/core/intent-quality-doctor.ts`·`src/core/completion-coverage-doctor.ts`. intent floor 공허(후속): `src/core/interview-driver.ts:70,271`. 설계 상세(drift 주의·비권위): `reports/design/floor-raising-blueprint.md`. 심의: dialectic round-1 (verdict=revise).

### 착지 (landed)
- **increment 1** (wi_260623uap): 결정 1·2·3 + 적대 변환기 + forward-AC frozen oracle = AC↔oracle 코어. fresh verifier+reviewer 검증.
- **increment 6** (wi_260623u0d, commit `47284f6`): 결정 6 루프 규율 — converged vs capped(decision-log disposition), loop-level cap(`caps.loop_rounds`), in-loop oracle authoritative(`gates.ts oracleSatisfaction`), wrong-fixpoint reopen(passed→pending + append-only decision), 동일 oracle K회 실패→blocked(`caps.oracle_failures_to_block`).
- **increment 3** (wi_260624bad, commit `79b2e33`): 결정 4·5·7 — design 노드 제자리 경화. **④회고 측정**=`src/core/retro-measure.ts` `assembleRetroMetrics`(①outcome_floor·②process_health **분리**, anti-SLOP omit, 투영-only 서술 cross-WI 흡수), retro 노드를 design-close에서 모든 신규 WI에 추가·**non-blocking**(`autopilot-driver.ts:113`·`autopilot-loop.ts` all_passed 제외). **⑤plan oracle 뷰**=`ditto autopilot status`가 AC↔oracle(method·maps_to·direction) 표시(read-only·pending일 때만 enrich, `cli/commands/autopilot.ts:1021`). **⑦의사결정 투명성**=루프 종료 disposition(converged|capped|blocked)을 decision-log 단일 SoT로 기록(`autopilot-store.ts:40` `loop_terminated`/`disposition`·`recordLoopTermination`).
- **K 카운터 per-AC** (wi_260624kcv, commit `79b2e33`): 결정 6의 oracle-failure K 카운터를 per-node→**per-AC**로 정교화(**별도 결정 아님 — 결정 6 메커니즘의 increment 잔여**). 구조화 `AutopilotDecision.criterion_ids`(`autopilot-store.ts:51`) + legacy node-scoped fallback, `sameOracleFailureCount`(`autopilot-loop.ts:994`).
- **정정**: increment 1 시점 '결정 7 부분 내재화(oracle-unmet reason 발화)'는 increment 3으로 **완전 착지**(루프 종료 disposition까지 기록).
- **검증**: 전체 `bun test` 2867 pass / 9 skip / 2 fail(2 fail = 이 glossary `oracle` 항목 >800자, 형제 커밋 c379043 유입 · 이 increment 무관 — wi_260624k67에서 동반 정리).
- **남은 갭(후속 increment B)**: retro 노드가 `autopilot complete`(completion.json 생산)보다 **먼저** 실행돼, retro가 측정하는 outcome_floor의 completion-기반 슬롯(coverage·unit_only_closures)이 그 시점 ungrounded → omit됨. escape_recurrence·post_cost는 grounded. 결정 4 outcome_floor 절반이 정상 흐름에서 측정 안 됨 → 순서 재배치 또는 측정 시점 이동 필요(별도 WI). 근거: `autopilot-loop.ts:1569`(retro 삽입)·`cli/commands/autopilot.ts`(complete는 done action 후)·`retro-measure.ts:87`(completion 부재 시 omit).
- 후속으로 남은 것: 측정값 추세 보존(시점 스냅샷 ledger) · 결정 5 미리보기 저위험 WI 확대(measure 후) · **retro↔completion 순서 갭**(위 '남은 갭', 별도 WI).
