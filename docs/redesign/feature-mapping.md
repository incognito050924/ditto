...
## 8. 정체성(역할) → 실현 기능 (백서 §1·§3)

백서 §1·§3의 "프로젝트를 지속적으로 가꾸는 동료 — 가드너·큐레이터·컨설턴트"가 지금 무엇으로 실현되는지. 백서는 기능-무관 정체성만 담으므로(백서 서문 원칙) 구체 기능 연결은 여기 둔다.

| 역할 | 하는 일 | 실현 기능 | 방침 |
|---|---|---|---|
| 동료 (colleague) | 전 생애주기에 걸친 지속적 협업 파트너 | 기능 4축 전체(의도·오케스트레이션·E2E·지식, §1) | [보존] |
| 가드너 (gardener) | 변경을 계약·게이트에 대어 코드베이스 품질과 프로젝트 진행을 돌봄 | ACG 거버넌스: `architecture`·`change-contract`·`impact`·`boundary`·`fitness`·`semantic`·`acg-review`·`change-map` (§4) | 역할 [보존] / CodeQL 단일엔진 [재배선] |
| 큐레이터·컨설턴트 (curator·consultant) | 결정·용어·지식을 코드 곁에 축적·정리해 살아있게 유지하고, 그 축적에 기대 방향을 조언 | memory 그래프 · glossary · ADR (§3 핵심 개념) | [보존] |

**향후 (roadmap — 현재 구현 아님)**: 위 두 역할(가드너의 품질 관리, 큐레이터·컨설턴트의 지식)을 비-DITTO 실프로젝트로 확장하는 **palimpsest 연계** — self 도그푸딩을 넘어 임의 사용자 프로젝트에서 같은 돌봄을 검증·발현한다(백서 §3 "가치는 사용자 프로젝트에서 발현" 정합). 아직 착수 전이라 방침 미정.

## 9. 이 매핑을 읽는 순서 (재설계 팀용)
