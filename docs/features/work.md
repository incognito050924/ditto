...
> 문서 성격: 코드 설명(재설계용). 코드 변경에 자동 동기화되지 않으므로 시간이 지나면 실제 코드와 어긋날 수 있다(권위는 코드에 있다). 기준 커밋 `c2d2e16`, 기준일 2026-07-19.
...
| `src/core/work-item-handoff.ts` | `writeWorkItemHandoff`·`collectChangedFiles`·`sanitizeDeclaredPaths` — 핸드오프 생성과 결정적 changed_files 수집. |
...
| `handoff <wi>` | 핸드오프 문서 생성/갱신, 또는 `--show`로 기존 문서 로드. | `--base` `--head` `--declared-by` `--show` `--remote` |
...
- **저장**: Record(record.json + events/), Run(completion.json·미러 등), 핸드오프(`.ditto/local/handoff/` 및 `--remote` 시 브랜치 `.ditto/handoff/`).
...
- `started_at_sha`(`:768`): status가 in_progress이고 아직 비어 있을 때만 git HEAD sha를 한 번 박는다(멱등). 핸드오프/done의 changed_files diff 기본 base가 된다.
...
6. **completion 합성 + 게이트**(`:2382`): `assembleCompletionFromWorkItem` → `completionGate` + `completionEvidenceGate`. final_verdict≠pass면 거부. 기존 completion이 pass면 재합성 skip(no churn), non-pass면 재합성해 supersede(`:2194` — 예: stale partial 핸드오프가 남긴 completion.json).
...
### 5-5. changed_files 수집 — `collectChangedFiles` (`work-item-handoff.ts:148`)

`base...HEAD` diff(`--diff-filter=ACMR`) ∪ declared를 `files`에 넣는다. `head`가 명시되면 워킹트리는 아예 참조 안 함(과거 커밋 범위 정정). `head===null`일 때만 `git status --porcelain`을 GUARD로 읽어, 커밋도 선언도 baseline도 아닌 tracked 편집을 `extraTrackedDirt`에 모은다(`:189`). 이 경로들은 **절대 `files`에 fold되지 않는다** — 그게 이 모듈이 고친 오염 버그의 핵심.

`sanitizeDeclaredPaths`(`:110`)는 `--changed` 입력을 `containScopePath`(절대경로·`..`탈출·pathspec-magic 거부)에 leading-`-` 거부(option-injection 방어)를 합쳐 정화한다.
...
### 5-8. 핸드오프 — `writeWorkItemHandoff` (`work-item-handoff.ts:281`)

completion.json을 합성하되, autopilot 그래프가 있으면 그래프-기반 verdict(`deriveAcVerdicts`)를 써서 `autopilot complete`와 **같은 소스로 일치**시킨다(재핸드오프가 좋은 pass를 stale partial로 덮어쓰지 못하게, `:385`). pass면 terminal close를 단일 chokepoint `store.close`로 라우팅(`:481`), 핸드오프 본문은 archive 직행(active 소음 0); non-pass면 status=partial + re_entry로 park.
...
- **`re_entry` hint가 존재하지 않는 명령을 가리키던 문제는 이미 수정**: 핸드오프의 재진입 hint가 `ditto work handoff <id> --show`(실존)를 가리킨다(`work-item-handoff.ts:425`) — 과거 `ditto work resume`(미존재) 참조를 교정.
