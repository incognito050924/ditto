...
repo를 clone할 필요 없다. **DITTO가 관리할 프로젝트(git repo)** 안에서 한 줄이면
Claude Code 플러그인 + 전역 `ditto` CLI를 설치하고 그 프로젝트의 `.ditto/`까지
scaffold한다 — `npx`가 GitHub 소스를 바로 받아온다(npm publish 없음):

```bash
npx github:incognito050924/ditto install
```

업데이트·제거도 같은 방식이다([업데이트 & 도그푸딩](#업데이트--도그푸딩),
[상태 & 제거](#상태--제거) 참고):

```bash
npx github:incognito050924/ditto update      # 최신으로 갱신
npx github:incognito050924/ditto uninstall   # 플러그인 + 전역 CLI 제거
```

**git repo 밖에서** 설치를 실행하면 전역 플러그인만 깔고 프로젝트 단계는 건너뛴다 —
그 뒤 프로젝트로 `cd`해서 `ditto setup`을 실행한다. npx 설치는 선택적 분석 도구
(CodeQL/Playwright/LSP)를 provisioning하지 않는다 — 나중에 `ditto setup --tools`로 추가한다.

### 로컬 clone에서 (기여자)

repo를 이미 받아둔 경우(DITTO 자체 개발 또는 도그푸딩)에는 install 스크립트를 쓴다 —
GitHub이 아니라 작업 트리에서 빌드한다:
...
**npx로 설치했다면?** `npx github:incognito050924/ditto update`로 갱신한다 —
marketplace + 플러그인을 갱신하고 `ditto setup`을 재실행한다(멱등). 이 섹션의 나머지는
**로컬 clone**(기여자/도그푸딩) 업데이트에 대한 것이다.

로컬 clone에는 전용 `ditto update` 명령이 없다 — **업데이트 = 부트스트랩 재실행**이다
(`install.sh`는 멱등: 재빌드 + 멱등 `ditto setup`). 더해 아래 `dist/plugin` 자동 재빌드가 있다. 설치된 플러그인은
...
제거는 **두 층**이다 — 어디까지 지울지에 따라 고른다:

```bash
# 특정 프로젝트에서만 (전역 플러그인 + CLI는 그대로 유지):
ditto uninstall                                  # 관리 블록 + allowlist 제거, .ditto/ 보존
ditto uninstall --purge                          # .ditto/까지 삭제(work-item 이력 + 메모리)

# 전역 호스트 설치(플러그인 + 전역 `ditto` CLI), npx로 깔았다면:
npx github:incognito050924/ditto uninstall       # 각 프로젝트의 .ditto/는 그대로 둠
```

`npx … uninstall`은 플러그인·marketplace 항목·전역 `ditto` 심링크를 제거하지만 어떤 프로젝트의
`.ditto/`도 **건드리지 않는다** — 그 데이터까지 지우려면 프로젝트마다 `ditto uninstall --purge`를 먼저 실행한다.

**로컬 clone**에서는 install 스크립트가 전역 teardown과 프로젝트 단계를 한 번에 처리한다:
...
스크립트는 바이너리 심링크를 제거하고 `ditto uninstall`(별칭: `teardown`)에 위임한다 — 관리 지침 블록과
