...
/**
 * Push-gate green-tree cache (wi_260706d0i) — lets the pre-push gate SKIP a
 * redundant full-suite re-run when the EXACT tree being pushed already passed the
 * gate's `test_command`. Safety is preserved by construction: a skip needs a CLEAN
 * working tree AND an exact git-tree-hash match, and a green record is only written
 * when the run command equals the gate command (a subset run can never authorize a
 * full-gate skip). Everything else falls back to running the full command.
 *
 ...
 * under the gitignored `.ditto/local/`, so it is per-machine and never committed.
 */
 ...
 */
export function shouldRecordGreen(
  commandRun: string,
  gateCommand: string,
  clean: boolean,
): boolean {
...
/** The gitignored, per-machine cache file for a repo rooted at `cwd`. */
export function greenCachePath(cwd: string): string {
  return join(cwd, '.ditto', 'local', 'push-gate-green.json');
}
