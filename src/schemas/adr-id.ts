/**
 * ADR-id grammar primitives — defined ONCE here so the schemas (`knowledge-record`,
 * `decision-conflict-carrier`), the knowledge-bridge projection, and the knowledge
 * CLI all share a single source for the id shape instead of re-spelling the regex.
 *
 * This module lives in `src/schemas/` (a leaf layer that imports no `src/core`)
 * precisely so core AND cli can depend on it without inverting the dependency
 * direction (schemas must not depend on core).
 *
 * Two id FORMS exist:
 *   - legacy `ADR-NNNN` (4-digit number; `knowledge.json` uses this), and
 ...
 *   - ANCHORED (`^...$`): validates a complete id. Order is irrelevant because the
 *     whole string must match, but the legacy-first spelling is preserved verbatim.
 *   - UNANCHORED (prefix extraction): the 8-digit(+slug) branch MUST come first.
 *     `\d{4}` is a prefix of `\d{8}`, so a `\d{4}`-first alternation would truncate
 *     `ADR-20260624-x` to `ADR-2026`.
...
/**
 * ANCHORED full-id validator for the schemas' `adrId` / `adr_id` fields: a complete
 * id is legacy `ADR-NNNN` or new `ADR-YYYYMMDD-<slug>`. (Legacy-first ordering;
 * anchored, so ordering does not affect matches.)
 */
export const ADR_ID_FULL_RE = new RegExp(`^ADR-(?:${DIGITS4}|${DIGITS8}-${SLUG})$`);

/**
 * UNANCHORED prefix extractor: pull the id out of a filename/stem. 8-digit branch
 * FIRST so a new-form date is not truncated to its first 4 digits. For legacy
 * `ADR-NNNN-<slug>.md` this yields `ADR-NNNN`; for `ADR-YYYYMMDD-<slug>.md` the
 * full `ADR-YYYYMMDD-<slug>`.
 */
...
/**
 * Extract an ADR's identifier from a filename, or null when the name is malformed.
 * Shared drop-in for the bridge's `adrHeadline` and the CLI's adr-check: both
 * gate on a whole well-formed filename, then take the id prefix.
 */
